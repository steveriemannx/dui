...
void MainForm::UpdateUI()
{
    ...
    pLabel = ui::Find<ui::Label>(this, "SDL_GetWindowSize");
    if (pLabel != nullptr) {
        int32_t w = 0;
        int32_t h = 0;
    ...
    pLabel = ui::Find<ui::Label>(this, "SDL_GetWindowSizeInPixels");
    if (pLabel != nullptr) {
        int32_t w = 0;
        int32_t h = 0;
    ...
    pLabel = ui::Find<ui::Label>(this, "SDL_GetDisplayContentScale");
    if (pLabel != nullptr) {
        float scale = NativeWnd()->GetDisplayContentScale();
        auto text = ui::StringUtil::Printf("%.02f", scale);
        pLabel->SetText(text);
    }
    pLabel = ui::Find<ui::Label>(this, "SDL_GetWindowDisplayScale");
    if (pLabel != nullptr) {
        float scale = NativeWnd()->GetWindowDisplayScale();
        auto text = ui::StringUtil::Printf("%.02f", scale);
        pLabel->SetText(text);
    }
    pLabel = ui::Find<ui::Label>(this, "SDL_GetWindowPixelDensity");
    if (pLabel != nullptr) {
...
#else
    ui::Control* pSDL = ui::Find<ui::Control>(this, "SDL");
    if (pSDL != nullptr) {
