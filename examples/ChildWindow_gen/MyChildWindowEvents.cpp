    ...
    m_pPaintScheduler(pPaintScheduler)
{
    ...
    m_pFpsLabel = dynamic_cast<ui::Label*>(pChildWindow->FindSubControl("label_fps"));
    m_bPaintFps = true;

    // Update the child window name
    ui::Label* pChildWindowName = dynamic_cast<ui::Label*>(pChildWindow->FindSubControl("child_window_name"));
    if (pChildWindowName != nullptr) {
        std::string childWindowName = ui::StringUtil::Printf("ChildWnd%d", (int32_t)nChildWindowIndex);
        pChildWindowName->SetText(childWindowName);
    }

    // Dynamic drawing
    ui::CheckBox* pStartFpsPaint = dynamic_cast<ui::CheckBox*>(pChildWindow->FindSubControl("fps_paint"));
    ...
    // Full-screen display
    ui::CheckBox* pFullscreen = dynamic_cast<ui::CheckBox*>(pChildWindow->FindSubControl("child_fullscreen"));
    ...
    if (m_pChildWindow != nullptr) {
        ...
        if (pWindow != nullptr) {
            pWindow->AttachWindowExitFullscreenMsg([this](const ui::EventArgs&) {
                if (m_pChildWindow != nullptr) {
                    ui::CheckBox* pFullscreen = dynamic_cast<ui::CheckBox*>(m_pChildWindow->FindSubControl("child_fullscreen"));
                    if (pFullscreen != nullptr) {
...
LRESULT MyChildWindowEvents::OnPaintMsg(const ui::UiRect& rcPaint, const ui::NativeMsg& nativeMsg, bool& bHandled)
{
    ...
    if (m_bPaintFps) {
        ...
        if (duration.count() >= 500) {
            ...
            std::string fps = ui::StringUtil::Printf("%d", (int32_t)m_fps->GetFPS());
