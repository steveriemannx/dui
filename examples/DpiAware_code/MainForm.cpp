...
void MainForm::BuildUI()
{
    ...
    auto* backendVBox = ui::Create<ui::VBox>(this, {{"name", "native backend"}, {"height", "auto"}, {"width", "100%"}, {"valign", "center"}, {"padding", "20,0,0,0"}, {"margin", "0,4,0,0"}});
    ui::Attach(pGroupBox, backendVBox);

    struct NativeRow { std::string name; std::string label; std::string init; };
    const NativeRow nativeRows[] = {
        { "Native_GetWindowSize", "Native_GetWindowSize：", "W:0,H:0" },
        { "Native_GetWindowSizeInPixels", "Native_GetWindowSizeInPixels：", "W:0,H:0" },
        { "Native_GetDisplayContentScale", "Native_GetDisplayContentScale：", "0" },
        { "Native_GetWindowDisplayScale", "Native_GetWindowDisplayScale：", "0" },
        { "Native_GetWindowPixelDensity", "Native_GetWindowPixelDensity：", "0" },
    };
    for (const auto& row : nativeRows) {
    auto* pNativeRow = ui::Create<ui::HBox>(this, {{"height", "auto"}});
        ui::Attach(backendVBox, pNativeRow);

        auto* pNativeLabel = ui::Create<ui::Label>(this, {{"width", "300"}, {"text_align", "right,vcenter"}, {"text", row.label}});
        ui::Attach(pNativeRow, pNativeLabel);

        auto* pNativeValue = ui::Create<ui::Label>(this, {{"name", row.name}, {"text", row.init}, {"width", "auto"}, {"text_align", "left,vcenter"}});
...
void MainForm::UpdateUI()
{
    ...
    if (auto* pLabel = ui::Find<ui::Label>(this, "Native_GetWindowSize")) {
        int32_t w = 0;
        int32_t h = 0;
    ...
    if (auto* pLabel = ui::Find<ui::Label>(this, "Native_GetWindowSizeInPixels")) {
        int32_t w = 0;
        int32_t h = 0;
    ...
    if (auto* pLabel = ui::Find<ui::Label>(this, "Native_GetDisplayContentScale")) {
        float scale = NativeWnd()->GetDisplayContentScale();
        auto text = ui::StringUtil::Printf("%.02f", scale);
        pLabel->SetText(text);
    }
    if (auto* pLabel = ui::Find<ui::Label>(this, "Native_GetWindowDisplayScale")) {
        float scale = NativeWnd()->GetWindowDisplayScale();
        auto text = ui::StringUtil::Printf("%.02f", scale);
        pLabel->SetText(text);
    }
    if (auto* pLabel = ui::Find<ui::Label>(this, "Native_GetWindowPixelDensity")) {
        float scale = NativeWnd()->GetWindowPixelDensity();
        auto text = ui::StringUtil::Printf("%.02f", scale);
        pLabel->SetText(text);
    }
#else
    if (auto* nativeBackend = ui::Find<ui::Control>(this, "native backend")) {
