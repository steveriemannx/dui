...
public:
    ...
    * @param [in] iconFilePath Icon file path (ico format supported)
...
private:
    ...
    */
    ...
    /** Window procedure function, used to receive tray icon messages
    */
    static LRESULT CALLBACK TrayIconWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam);

private:
    /** Handle of the message window
    */
    HWND m_hWnd;
...
bool TrayIconImpl::Initialize(const Window* pWindow, const DString& iconFilePath, const DString& tooltip)
{
    ...
    // Store the this pointer in the window properties
...
HICON TrayIconImpl::LoadIconFromFile(const Window* pWindow, const DString& iconFilePath)
{
    ...
    if (GlobalManager::Instance().Zip().IsUseZip() &&
        ...
        // Try to load the icon from the resource path
...
bool TrayIconImpl::UpdateTrayIcon(DWORD dwMessage)
{
    ...
    if (!result && dwMessage == NIM_ADD) {
        // If adding fails, try deleting first and then adding again
...
LRESULT CALLBACK TrayIconImpl::TrayIconWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (uMsg == WM_TRAYICON_MESSAGE) {
        ...
        switch (uMouseMsg) {
        case WM_LBUTTONDOWN:
            // Left button pressed, mark the state
            pTrayIcon->m_bLeftButtonDown = true;
            break;
        case WM_LBUTTONUP:
            // Left button released, check whether the press was marked
        ...
        case WM_LBUTTONDBLCLK:
            // Double-click event (on double-click, no second DOWN message is received; DBLCLK is received directly)
            pTrayIcon->m_bLeftButtonDown = false;
            pTrayIcon->NotifyMessage(TrayIconMessageType::kLeftDoubleClick, pt.x, pt.y);
            break;
        case WM_RBUTTONDOWN:
            // Right button pressed, mark the state
            pTrayIcon->m_bRightButtonDown = true;
            break;
        case WM_RBUTTONUP:
            // Right button released, check whether the press was marked
            ...
            break;
        case WM_CONTEXTMENU:
            // Context menu message (some systems send this message directly)
            // Reset the right-button flag to avoid duplicate triggers
            pTrayIcon->m_bRightButtonDown = false;
            pTrayIcon->NotifyMessage(TrayIconMessageType::kRightClick, pt.x, pt.y);
            break;
        case WM_RBUTTONDBLCLK:
            ...
            break;
        case WM_MBUTTONDOWN:
            // Middle button pressed, mark the state
            pTrayIcon->m_bMiddleButtonDown = true;
            break;
        case WM_MBUTTONUP:
            // Middle button released, check whether the press was marked
