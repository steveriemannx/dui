...
IRender* BitmapHelper::CreateRenderObject(int srcRenderWidth, int srcRenderHeight, HDC hSrcDc, int srcDcWidth, int srcDcHeight)
{
    ...
    // Copy the bitmap to the memory DC
...
HBITMAP BitmapHelper::CreateGDIBitmap(int32_t nWidth, int32_t nHeight, bool flipHeight, LPVOID* pBits)
{
    ...
    if (flipHeight) {
        bmi.bmiHeader.biHeight = -nHeight;//A negative value means the bitmap orientation is top-to-bottom, with the origin at the top-left corner
    }
    else {
        bmi.bmiHeader.biHeight = nHeight; //A positive value means the bitmap orientation is bottom-to-top, with the origin at the bottom-left corner
    }
