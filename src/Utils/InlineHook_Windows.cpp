...
// Dynamically parse the instruction boundaries and return the covered length
static size_t CalculateHookLengthWithUdis86(void* target_addr, size_t min_hook_bytes)
{
    ...
    ud_set_input_buffer(&ud_ctx, (uint8_t*)target_addr, 64); // Scan the first 64 bytes
...
bool InlineHook::DoInstall()
{
    ...
    //Calculate the minimum length of the trampoline (must be greater than the length of m_jumpCode)
...
bool InlineHook::CreateTrampoline()
{
    //Key implementation details (x64):
    //1. Jump instruction construction
    //(1) Uses the mov rax, address + jmp rax combination to perform a 64-bit address jump
    //(2) Occupies 14 bytes in total (48 B8 [8-byte address] FF E0)
    //2. Trampoline structure
    //   [The first 14 bytes of instructions of the original function]
    //   [mov rax, original function + 14]
    //   [jmp rax]
    ...
    // Generate the jump-back instructions
#ifdef _M_X64
    ...
    // Write the jump-back instructions
