...
{
    if (pstrName == DUI_T("vscrollbar")) {
        EnableScrollBar(pstrValue == DUI_T("true"), GetHScrollBar() != nullptr);
    }
    else if ((pstrName == DUI_T("vscrollbar_style")) || (pstrName == DUI_T("vscrollbarstyle"))) {
        EnableScrollBar(true, GetHScrollBar() != nullptr);
        if (GetVScrollBar() != nullptr) {
            GetVScrollBar()->ApplyAttributeList(pstrValue);
        }
    }
    else if (pstrName == DUI_T("vscrollbar_class")) {
        EnableScrollBar(true, GetHScrollBar() != nullptr);
        if (GetVScrollBar() != nullptr) {
            GetVScrollBar()->SetClass(pstrValue);
        }
    }
    else if (pstrName == DUI_T("hscrollbar")) {
        EnableScrollBar(GetVScrollBar() != nullptr, pstrValue == DUI_T("true"));
    }
    else if ((pstrName == DUI_T("hscrollbar_style")) || (pstrName == DUI_T("hscrollbarstyle"))) {
        EnableScrollBar(GetVScrollBar() != nullptr, true);
        if (GetHScrollBar() != nullptr) {
            GetHScrollBar()->ApplyAttributeList(pstrValue);
        }
    }
    else if (pstrName == DUI_T("hscrollbar_class")) {
        EnableScrollBar(GetVScrollBar() != nullptr, true);
        if (GetHScrollBar() != nullptr) {
            GetHScrollBar()->SetClass(pstrValue);
        }
    }
    else if ((pstrName == DUI_T("scrollbar_padding")) || (pstrName == DUI_T("scrollbarpadding"))) {
    ...
    else if ((pstrName == DUI_T("vscroll_unit")) || (pstrName == DUI_T("vscrollunit"))) {
        int32_t iValue = StringUtil::StringToInt32(pstrValue);
        SetVerScrollUnitPixels(iValue, true);
    }
    else if ((pstrName == DUI_T("hscroll_unit")) || (pstrName == DUI_T("hscrollunit"))) {
        int32_t iValue = StringUtil::StringToInt32(pstrValue);
        SetHorScrollUnitPixels(iValue, true);
    }
    else if ((pstrName == DUI_T("scrollbar_float")) || (pstrName == DUI_T("scrollbarfloat"))) {
        SetScrollBarFloat(pstrValue == DUI_T("true"));
    }
    else if ((pstrName == DUI_T("vscrollbar_left")) || (pstrName == DUI_T("vscrollbarleft"))) {
        SetVScrollBarAtLeft(pstrValue == DUI_T("true"));
    }
    else if ((pstrName == DUI_T("hold_end")) || (pstrName == DUI_T("holdend"))) {
        SetHoldEnd(pstrValue == DUI_T("true"));
    }
    else {
...
void ScrollBox::SetPosInternally(const UiRect& rc, bool bScrollProcess)
{
    ...
    if (pLayout->LayoutByActualAreaSize()) {
        //In a container that supports a scroll bar, the layout of stretched child controls depends on the size of the target area, so the target area size needs to be calculated in advance
        requiredSize = CalcRequiredSize(rc, true);//Only calculate the size and position of the child controls, without adjusting them
        if ((requiredSize.cx > 0) && (requiredSize.cy > 0)) {
            ...
            if ((realRect.Width() != rc.Width()) || (realRect.Height() != rc.Height())) {
                requiredSize = CalcRequiredSize(realRect, false); //Calculate the size and position of the child controls, and adjust them
                bArrangedChildren = true;
            }
        }
    }
    if (!bArrangedChildren) {
        requiredSize = CalcRequiredSize(rc, false);//Calculate the size and position of the child controls, and adjust them
    }

    //requiredSize needs to subtract the inner margin, consistent with the logic of ProcessVScrollBar/ProcessHScrollBar
    ...
    //Determine whether the control position needs to be reset (when the scroll bar state changes, it needs to be reset)
    bool bNeedResetPos = false;
    if (m_pVScrollBar != nullptr) {
        if (m_pVScrollBar->IsValid()) {
            if (!bShowVScrollBar) {
                //Scroll bar state change: from visible to hidden
            ...
            if (bShowVScrollBar) {
                //Scroll bar state change: from hidden to visible
    ...
    if (bNeedResetPos && !bScrollProcess) {
        //Need to reset the position and size (because the scroll bar's visible/hidden state changed, the actual content area has changed)
        DoSetPos(rc, true);
    }
    else {
        bNeedResetPos = false;
...
UiSize64 ScrollBox::DoArrangeChildren(const UiRect& rc, bool bEstimateOnly)
{
    ...
    if (!m_bScrollBarFloat && (m_pVScrollBar != nullptr) && m_pVScrollBar->IsValid()) {
        //Subtract the width of the vertical scroll bar from the target area
    ...
    if (!m_bScrollBarFloat && (m_pHScrollBar != nullptr) && m_pHScrollBar->IsValid()) {
        //Subtract the height of the horizontal scroll bar from the target area
        ASSERT(m_pHScrollBar->GetFixedHeight().GetInt32() > 0);
        childSize.bottom -= m_pHScrollBar->GetFixedHeight().GetInt32();
    }
    childSize.Validate();

    //In the target area, perform layout management on the child controls (adjust the position and size of the child controls)
...
void ScrollBox::ProcessVScrollBar(UiRect rcScrollBarPos, int64_t nScrollRange, bool bShowHScrollBar, bool& bNeedResetPos)
{
    ...
    int32_t nHScrollbarHeight = 0; //Height of the horizontal scroll bar
    if ((m_pHScrollBar != nullptr) && bShowHScrollBar) {
        //The bottom of the vertical scroll bar needs to reach the bottom of the container
...
void ScrollBox::ProcessHScrollBar(UiRect rcScrollBarPos, int64_t nScrollRange, bool bShowVScrollBar, bool& bNeedResetPos)
{
    ...
    //Position of the vertical scroll bar: the horizontal scroll bar position must not cover the vertical scroll bar position
    ...
    if( (m_pVScrollBar != nullptr) && m_pVScrollBar->IsValid() && m_pVScrollBar->IsEnabled() ) {
        if(msg.eventType == kEventKeyDown ) {
            ...
            if (msg.lParam != 0) {
                ...
                //Scroll strictly according to the passed-in parameters
                if (deltaValue > 0) {
...
void ScrollBox::PaintChild(IRender* pRender, const UiRect& rcPaint)
{
    ...
    for (Control* pControl : m_items) {
        ...
        if (pControl->GetPaintOrder() != 0) {
            //A paint order is set, add it to the delayed paint list
    ...
    if (!delayItems.empty()) {
        ...
        //Paint the controls in the delayed paint list
...
{
    //ptMouse: the coordinate adapted to the container itself
    //scrollPos: the scroll bar offset of the current container
...
void ScrollBox::EnableScrollBar(bool bEnableVertical, bool bEnableHorizontal)
{
    if( bEnableVertical && (m_pVScrollBar == nullptr) ) {
        ...
        m_pVScrollBar->SetClass(DUI_T("vscrollbar"));
    }
    ...
    if( bEnableHorizontal && (m_pHScrollBar == nullptr)) {
        ...
        m_pHScrollBar->SetClass(DUI_T("hscrollbar"));
    }
...
{
    //The virtual scroll bar position is introduced to solve the problem that UiRect with 32-bit integer values cannot support extremely large virtual tables (tens of millions of data items or more)
    UiSize64 scrollPos = GetScrollPos();
