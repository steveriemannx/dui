...
*/
bool GetText(Control* pControl, std::string& strValue)
{
...
bool GetValue(Control* pControl, std::string& strValue)
{
    Progress* pProgress = dynamic_cast<Progress*>(pControl);
    if (pProgress == nullptr) {
        return false;
    }
    strValue = StringUtil::Printf("%g", pProgress->GetValue());
    return true;
}

bool GetVisible(Control* pControl, std::string& strValue)
{
    strValue = pControl->IsVisible() ? "true" : "false";
    return true;
}

bool GetEnabled(Control* pControl, std::string& strValue)
{
    strValue = pControl->IsEnabled() ? "true" : "false";
    return true;
}

bool GetBkImage(Control* pControl, std::string& strValue)
{
...
const std::vector<PropertyEntry>& GetPropertyTable()
{
    static const std::vector<PropertyEntry> kTable = {
        ...
        { "text",    { EventType::kEventTextChanged,    &GetText } },
        // Readable on Progress and Slider; Slider reports changes, Progress does not.
        { "value",   { EventType::kEventValueChanged,   &GetValue } },
        // Readable on every Control, and every Control reports visibility changes.
        { "visible", { EventType::kEventVisibleChanged, &GetVisible } },
        // Readable but unreported: these are one-way only in practice.
        { "enabled", { EventType::kEventNone,           &GetEnabled } },
        { "bkimage", { EventType::kEventNone,           &GetBkImage } },
    };
    return kTable;
}
