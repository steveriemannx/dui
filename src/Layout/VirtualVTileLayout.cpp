...
VirtualVTileLayout::VirtualVTileLayout()
{
    ...
    // Default to horizontal direction, centered alignment
...
UiSize64 VirtualVTileLayout::ArrangeChildren(const std::vector<ui::Control*>& items, ui::UiRect rc, bool bEstimateOnly)
{
    ...
    if ((pList == nullptr) || !pList->HasDataProvider()) {
        // If no data interface is set, fall back to the base class functionality
        return BaseClass::ArrangeChildren(items, rc, bEstimateOnly);
    }
    DeflatePadding(rc);
    if ((GetColumns() > 0) && IsAutoCalcItemWidth()) {
        // A fixed column count is set, and auto-calculation of the child item width is enabled
    ...
    // Calculate the required horizontal width to support the horizontal scrollbar
    int32_t nColumns = CalcTileColumns(rc.Width()); // number of columns
...
{
    // When estimating the control size (mainly for "auto" width/height cases), only estimate the container's own size, excluding the size of the list data
    // Since the virtual list data can be large, it is not suitable for estimating the size of "auto" controls
    VirtualListBox* pList = dynamic_cast<VirtualListBox*>(GetOwner());
    if ((pList == nullptr) || !pList->HasDataProvider()) {
        // If no data interface is set, fall back to the base class functionality
...
int64_t VirtualVTileLayout::GetElementsHeight(const UiRect& rc, size_t nCount) const
{
    ...
    if (nCount <= (size_t)nColumns && nCount != Box::InvalidIndex) {
        // Less than one row, or exactly one row
...
void VirtualVTileLayout::LazyArrangeChild(UiRect rc) const
{
    ...
    // Left starting position of the child items
    ...
    // The Y-axis coordinate offset must be preserved to avoid display offset after the interface is refreshed when the scroll position changes
    int32_t yOffset = 0;
    ...
    // Top starting position of the child items
    int32_t iPosTop = rc.top - yOffset;

    // Set the virtual offset; otherwise, when the data volume is large, the height of the 32-bit rect rc may overflow and require a 64-bit integer
    pOwnerBox->SetScrollVirtualOffsetY(pOwnerBox->GetScrollPos().cy);

    // Top-left corner coordinates of the control
    ...
    for (size_t nItemIndex = 0; nItemIndex < nItemCount; ++nItemIndex) {
        ...
        if (nElementIndex < pOwnerBox->GetElementCount()) {
            ...
            // Need to clear the ElementIndex
...
size_t VirtualVTileLayout::AjustMaxItem(UiRect rc) const
{
    ...
    // Add one extra row to ensure the real controls fill the entire display area
    nRows += 1;
...
bool VirtualVTileLayout::IsElementDisplay(UiRect rc, size_t iIndex) const
{
    ...
    UiSize szItem = GetItemSize();
    if ((nElementPos - szItem.cy) > nScrollPos) { // top position of the rect
        int64_t nBoxHeight = pOwnerBox->GetHeight();
        if (nElementPos <= (nScrollPos + nBoxHeight)) {// bottom position of the rect
            // Fully displayed
            return true;
        }
    }
