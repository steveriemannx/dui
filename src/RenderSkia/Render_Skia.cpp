...
UiPoint Render_Skia::OffsetWindowOrg(UiPoint ptOffset)
{
    ...
    //Skia internally uses the SetViewportOrgEx logic (opposite of SetWindowOrgEx), so the sign is inverted here
...
/** Calculate how many times the image should be drawn in a loop when tiled (horizontal or vertical drawing)
* @param [in] nAvailableSpace The total available drawing space (width or height)
* @param [in] nImageSize The size of the image (width or height)
* @param [in] nTiledMargin The image spacing between two consecutive draws; no spacing at the far left (or top)
* @param [in] bFullyTiled true means each image draw must ensure the image is fully displayed; if it cannot be fully displayed, it is not drawn. false means the image need not be fully drawn; drawing part of it is allowed
*/
static int32_t CalcDrawImageTimes(int32_t nAvailableSpace, int32_t nImageSize, int32_t nTiledMargin, bool bFullyTiled)
{
    // Implementation approach
    //    Boundary condition handling: when the available space is <= 0 or the image size is <= 0, drawing is impossible and 0 is returned.
    //    Single draw check: determine whether the first image can be drawn (considering whether it must be fully displayed). If it cannot be drawn, return 0 directly.
    //    Multiple draw calculation: if the first image can be drawn, calculate the remaining space. For the remaining space, each draw must account for the image size and the margin (margins apply from the second draw onward); the total number of draws is determined by whether the image must be fully displayed.

    // Boundary condition: drawing is impossible when the available space or image size is invalid
    if (nAvailableSpace <= 0 || nImageSize <= 0) {
        return 0;
    }

    // Space required for the first image (no margin needed)
    int32_t firstImageRequired = bFullyTiled ? nImageSize : 1; // For partial drawing, 1 pixel is enough to draw
    if (nAvailableSpace < firstImageRequired) {
        return 0; // Even the first image cannot be drawn
    }

    // Already drawn once; subtract the space occupied by the first image from the remaining space
    int32_t remainingSpace = nAvailableSpace - nImageSize;
    int32_t drawTimes = 1;

    // When the remaining space is insufficient, return 1 directly
    if (remainingSpace <= 0) {
        return drawTimes;
    }

    // The leading margin for each subsequent draw (margins are needed from the second draw onward)
    while (true) {
        // Deduct the margin first (if the margin is insufficient, drawing cannot continue)
        if (remainingSpace < nTiledMargin) {
            break;
        }
        remainingSpace -= nTiledMargin;

        // Check whether the remaining space can support one draw (whether complete or not)
        if (remainingSpace <= 0) {
            break;
        }

        // Partial drawing: as long as the remaining space > 0, one draw is possible (even if only 1 pixel is shown)
        // Full drawing: the remaining space must be >= the image size
        if ((!bFullyTiled) || (remainingSpace >= nImageSize)) {
            drawTimes++;
            remainingSpace -= nImageSize; // Deduct the space occupied by the image (whether fully displayed or not)
        }
        else {
            break; // In full drawing mode, the remaining space is insufficient, cannot continue
        }
    }
                            ...
                            bool bWindowShadowMode)
{
    ...
    // The rectangle range of the middle area of the drawing target
    ...
    if (bTiledX || bTiledY) {
        //Middle part: subtract the tiling inner margin
        rcDrawDest.Deflate(rcTiledPadding);
    }

    // The source area of the drawn image (middle part)
    ...
    if (rcDestCorners.IsZero()) {
        //Disabled when not nine-patch drawing, to avoid the image not being drawn
    ...
    if (!bWindowShadowMode && UiRect::Intersect(rcTemp, rcPaint, rcDrawDest)) {
        ...
        if (!bTiledX && !bTiledY) {
            ...
            //Tiled: draw tiled both horizontally and vertically
            ...
            //Only horizontal tiling; the vertical direction is drawn by stretching
            const int32_t nImageWidth = rcSource.Width() - rcSourceCorners.left - rcSourceCorners.right;
            const int32_t nAvailableSpaceX = rcDrawDest.Width();
            const int32_t iTimesX = CalcDrawImageTimes(nAvailableSpaceX, nImageWidth, nTiledMarginX, bFullTiledX);
            ...
            for (int32_t i = 0; i < iTimesX; ++i) {
                ...
                //Source area: if corners are set, only the middle area is included
                rcDrawSource.left = rcSource.left + rcSourceCorners.left;
                rcDrawSource.top = rcSource.top + rcSourceCorners.top;
                rcDrawSource.right = rcDrawSource.left + lDrawWidth;
                rcDrawSource.bottom = rcSource.bottom - rcSourceCorners.bottom;
            ...
            //Only vertical tiling; the horizontal direction is drawn by stretching
            const int32_t nImageHeight = rcSource.Height() - rcSourceCorners.top - rcSourceCorners.bottom;
            const int32_t nAvailableSpaceY = rcDrawDest.Height();
            const int32_t iTimesY = CalcDrawImageTimes(nAvailableSpaceY, nImageHeight, nTiledMarginY, bFullTiledY);
                                ...
                                uint8_t uFade, IMatrix* pMatrix)
{
    ASSERT((GetWidth() > 0) && (GetHeight() > 0));
    if (pMatrix == nullptr) {
        //Only check the clip region when there is no Matrix,
        //because with a Matrix, the actual drawing area is not rcDest but the transformed position, and the checking method needs to be adjusted
        UiRect rcTestTemp;
        if (!UiRect::Intersect(rcTestTemp, rcDest, rcPaint)) {
            return;
        }
...
void Render_Skia::DrawRect(const UiRectF& rc, UiColor penColor, float fWidth, bool bLineInRect)
{
    ...
    SkRect rcSkDest = SkRect::MakeLTRB(rc.left, rc.top, rc.right, rc.bottom);
    if (bLineInRect) {
        //Ensure the drawn line stays within the rectangle
...
void Render_Skia::DrawString(const DString& strText, const DrawStringParam& drawParam)
{
    ...
    if (drawParam.uFormat & TEXT_VERTICAL) {
        ...
        //When horizontal text is set to justified alignment, or when word spacing is set, use this implementation (because modifying the SkTextBox implementation is difficult and hard to maintain)
...
UiRect Render_Skia::MeasureString(const DString& strText, const MeasureStringParam& measureParam)
{
    ...
    if (bSingleLineMode) {
        ...
        float spacingMul = 1.0f;//Row spacing multiplier; setting is not supported yet
                                ...
                                uint8_t uAlpha)
{
    ...
    //The original area, used for clipping
    SkRect excludeRc;
    ...
    //Set the attributes for drawing the shadow
    ...
    //Set the offset for drawing the shadow
    ...
    if (skCanvas != nullptr) {
        if (skCanvas->isClipEmpty()) {
            ...
            if (skCanvas->getLocalClipBounds(&rcClip)) {
                ...
                rect.Deflate(1, 1); //Note: it needs to be shrunk inward by one pixel (Skia enlarges by one pixel when setting the clip)
