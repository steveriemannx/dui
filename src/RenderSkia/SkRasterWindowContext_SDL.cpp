...
{
    PerformanceStat statPerformance(("PaintWindow, SkRasterWindowContext_SDL::SwapPaintBuffers"));
    ...
    // The window is transparent (SDL_WINDOW_TRANSPARENT): the shadow margin is
    // only a window if the clear leaves it that way. SDL's default draw colour is
    // opaque black, which is what a clear would otherwise paint there.
...
bool SkRasterWindowContext_SDL::SwapPaintBuffersFast(const UiRect& rcPaint, uint8_t nLayeredWindowAlpha)
{
    ...
    //Measure performance
    PerformanceStat statPerformance(("PaintWindow, SkRasterWindowContext_SDL::SwapPaintBuffersFast"));
