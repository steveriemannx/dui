...
{
    //The index in richTextData
    uint32_t m_nDataIndex = 0;

    //Physical line number
    uint32_t m_nLineNumber = 0;

    //Row number (the logical row number after drawing; when wrapped automatically, one physical line displays as multiple logical rows; the physical line number refers to the line number divided by the newline characters in the text)
    uint32_t m_nRowIndex = 0;

    //The text to be drawn
    std::wstring_view m_textView;

    //The destination area to draw
    UiRect m_destRect;
                                        ...
                                        std::vector<std::vector<UiRect>>* pRichTextRects)
{
    ...
    //Uses string_view internally to avoid string copying affecting performance
    ...
    //Drawing area: the coordinates of the drawing area use (rcTextRect.left, rcTextRect.top) as the (0,0) point
    ...
    if ((pLineInfoParam != nullptr) || (pDrawRichTextCache != nullptr)) {
        ...
        //Use the (0,0) coordinate point as the estimated (0,0) point
    ...
    const int32_t nTextRectRightMax = (int32_t)rcTextRect.right;   //The rightmost side of the drawing area
    const int32_t nTextRectBottomMax = (int32_t)rcTextRect.bottom; //The bottommost side of the drawing area

    SkScalar xPos = (SkScalar)rcDrawRect.left;  //Horizontal coordinate: when drawing characters, floating-point coordinates are used; the width occupied by each character is floating-point and cannot be aligned to pixels
    int32_t yPos = rcDrawRect.top;              //Vertical coordinate, aligned to pixels, so an integer is used
    int32_t nRowHeight = 0;   //Row height (the maximum drawing height of all characters in this row, aligned to pixels)
    uint32_t nLineNumber = 0; //Physical line number
    uint32_t nRowIndex = 0;   //Logical row number

    std::unordered_map<uint32_t, uint32_t> rowHeightMap;  //The actual row height table of each row

    //Font cache (since creating fonts is time-consuming, try to reuse the same object)
    ...
    if (pLineInfoParam != nullptr) {
        ...
        //The starting logical row number
    ...
    std::vector<SkGlyphID> glyphs;      //Internal temporary variable; declared outside the loop to improve execution speed
    std::vector<uint8_t> glyphChars;    //Internal temporary variable; declared outside the loop to improve execution speed
    std::vector<SkScalar> glyphWidths;  //Internal temporary variable; declared outside the loop to improve execution speed

    std::vector<uint8_t> glyphCharList;   //How many characters each glyph is made of
    std::vector<SkScalar> glyphWidthList; //The width of each character

    //Split the text by newline characters
    std::vector<std::wstring_view> lineTextViewList;

    //Internal temporary variable for line splitting; declared outside the loop to improve execution speed
    std::vector<uint32_t> lineSeprators;

    //Whether a TAB key is being drawn (aligned to 4 characters)
    bool bDrawTabChar = false;

    //How many characters this row (logical row) has drawn so far (excluding carriage returns and newlines)
    ...
    for (size_t index = 0; index < richTextData.size(); ++index) {
        ...
        if ((spLastSkiaFont != nullptr) &&
            ...
            //Reuse the font object in the cache
        ...
        SkScalar fFontHeight = skFont.getMetrics(&metrics);     //Font height, used when wrapping
        fFontHeight = textData.m_fRowSpacingMul * fFontHeight + textData.m_fRowSpacingAdd; //Apply the row spacing multiplier and row spacing addition
        const int32_t nFontHeight = SkScalarCeilToInt(fFontHeight);   //Row height aligned to pixels
        nRowHeight = std::max(nRowHeight, nFontHeight);
        if (nRowHeight <= 0) {
            continue;
        }
        const uint32_t uTextStyle = textData.m_textStyle;
        const bool bSingleLineMode = (uTextStyle & DrawStringFormat::TEXT_SINGLELINE) ? true : false; //Whether it is single-line mode; in single-line mode, no wrapping
        const bool bWordWrap = bSingleLineMode ? false : ((uTextStyle & DrawStringFormat::TEXT_WORD_WRAP) ? true : false);
        bool bBreakAll = false;//Marks whether to terminate

        //Split the text by newline characters
        lineTextViewList.clear();
        SplitLines(textData.m_textView, lineSeprators, lineTextViewList);

        //The logical row number within the physical line (numbered from 0 in each physical line)
        ...
        for (const std::wstring_view& lineTextView : lineTextViewList) {
            ...
            if (lineTextView.size() == 1) {              
                ...
                if (lineTextView[0] == L'\t') {
                    ...
                    //Wrap: perform the wrapping operation (physical wrap)
                    if (!bSingleLineMode) {
                    ...
                    continue; //Handle the next line
                }
            }

            //The starting text index to draw
            const size_t textCount = lineTextView.size();
            size_t textStartIndex = 0;
            while (textStartIndex < textCount) {
                //Estimate the text drawing area                
                size_t byteLength = (textCount - textStartIndex) * textCharSize;                
                SkScalar maxWidth = SkIntToScalar(rcDrawRect.right) - xPos;//Available width
                if (!bWordWrap || bSingleLineMode) {
                    //No auto-wrap, or single-line mode
                ...
                SkScalar textMeasuredWidth = 0;  //The estimated required width of the text currently to be drawn
                SkScalar textMeasuredHeight = 0; //The estimated required height of the text currently to be drawn

                glyphCharList.clear();
                glyphWidthList.clear();

                //Evaluate the rectangle range of each character
                ...
                if (bDrawTabChar) {
                    ...
                    //The breakText function takes about 30% of the execution time
                ...
                if (nDrawLength == 0) {
                    if (!bWordWrap || bSingleLineMode || (SkScalarTruncToInt(maxWidth) == rcDrawRect.Width())) {
                        //An error occurred (cannot wrap, or still insufficient after wrapping)
                        bBreakAll = true;
                        break;
                    }
                }
                    ...
                    //The rectangle area required for drawing the text
                    spTextData->m_destRect.left = SkScalarTruncToInt(xPos); //Left value: truncate directly; if there is a fractional part, just drop it

                    SkScalar fRight = xPos + textMeasuredWidth;             //Right value: if there is a fractional part, 1 pixel needs to be added
                    spTextData->m_destRect.right = SkScalarCeilToInt(fRight);
                    spTextData->m_destRect.top = yPos;
                    spTextData->m_destRect.bottom = yPos + SkScalarCeilToInt(textMeasuredHeight); //Record the actual height of the characters
                    ...
                    if (pLineInfoParam != nullptr) {
                        ...
                        if (glyphCharList.size() == glyphWidthList.size()) {
                            ...
                            for (size_t glyphIndex = 0; glyphIndex < glyphCount; ++glyphIndex) {
                                glyphWidth = glyphWidthList[glyphIndex];//Character width
                                glyphCharCount = glyphCharList[glyphIndex];  //How many characters this glyph occupies (UTF16 encoding, may be 1 or 2)
                    ...
                    //Count how many characters this logical row has drawn
                    nRowCharCount += glyphs.size();
                }

                bool bNextRow = false; //Flag for whether wrapping is needed
                if (nDrawLength < byteLength) {
                    //Insufficient width, wrapping is needed
                    if (!bSingleLineMode) {
                        bNextRow = true;
                    }                    
                    ...
                    //The current row can accommodate the text drawing
                    textStartIndex = textCount;//Marker to end the loop
                    ASSERT(((int64_t)xPos + (int64_t)textMeasuredWidth) < INT32_MAX);
                    xPos += textMeasuredWidth;
                    if (xPos >= nTextRectRightMax) {
                        //The right side of the X coordinate has exceeded the bounds of the destination rectangle
                        if (bBreakWhenOutOfRect && bSingleLineMode) {
                            //Single-line mode, terminate drawing
                            bBreakAll = true;
                            break;
                        }
                        else if (bWordWrap && !bSingleLineMode) {
                            //Wrap when auto-wrap is enabled
                            bNextRow = true;
                        }
                    }
                }
                ...
                if (bNextRow) {
                    //Wrap: perform the wrapping operation (logical wrap, nLineNumber is not incremented)
                    xPos = (SkScalar)rcDrawRect.left;
                    ASSERT(((int64_t)yPos + (int64_t)nRowHeight) < INT32_MAX);
                    yPos += nRowHeight;
                    rowHeightMap[nRowIndex] = nRowHeight;
                    ...
                    if (bBreakWhenOutOfRect && (yPos >= nTextRectBottomMax)) {
                        //The bottom of the Y coordinate has exceeded the bounds of the destination rectangle; terminate drawing
                        bBreakAll = true;
                        break;
                    }
                }
    ...
    //Record the row height of the last row
    rowHeightMap[nRowIndex] = nRowHeight;

    //Update the row height of each row (only by determining the row height in advance can vertically aligned text be drawn correctly)
    ...
    if (pRichTextRects != nullptr) {
        ...
        for (const SharePtr<TPendingDrawRichText>& spTextData : pendingTextData) {
            ...
            //Save the drawn destination areas; the same text may have multiple areas (when wrapped)
    ...
    if (pDrawRichTextCache != nullptr) {
        //Generate the draw cache, but do not perform drawing
        ...
        *pDrawRichTextCache = spDrawRichTextCache;
        ...
        for (const SharePtr<TPendingDrawRichText>& spTextData : pendingTextData) {
            ...
            //Draw the background color of the text
            ...
            if ((textData.m_textView.size() == 1) && (textData.m_textView[0] == L'\t')) {
                ...
                const size_t len = 1; //The number of bytes
                ...
                const size_t len = textData.m_textView.size() * textCharSize; //The number of bytes
...
void DrawRichText::SplitLines(const std::wstring_view& lineText, std::vector<uint32_t>& lineSeprators, std::vector<std::wstring_view>& lineTextViewList)
{
    ...
    if (lineSeprators.empty()) {
        //No newline separators, single line
        ...
        for (size_t nLine = 0; nLine < nLineSepCount; ++nLine) {
            ...
            if (nLine == (nLineSepCount - 1)) {
                //Last line: add the data of the last line
...
{
    //This function is called very frequently, once for each character; performance-sensitive    
    if (pLineInfoParam == nullptr) {
    ...
    size_t nIndex = nLineTextIndex; //The caller has already added the value of pLineInfoParam->m_nStartLineIndex    
    ...
    if (!bFound) {
        //The first character of this row
                                            ...
                                            const std::shared_ptr<DrawRichTextCache>& spDrawRichTextCache)
{
    ...
    if ((spDrawRichTextCache->m_textRect.Width() != textRect.Width()) ||
        ...
        //The rectangle size has changed, so the cache cannot be used (the cache can be used when only the position changes)
        return false;
    }
                                           ...
                                           const std::vector<int32_t>& rowRectTopList)
{
    ...
    //Remove the data of the deleted lines
    ...
    //The starting element index of the cache update    
    size_t nUpdateCacheStartIndex = (size_t)-1;
    //Whether the modified content has been appended to the end
    ...
    if (spUpdateDrawRichTextCache != nullptr) {
        ...
        if (!updateData.m_pendingTextData.empty()) {//The container may be empty (empty when this line is an empty line)
            ...
            //Merge the new draw cache into the original draw cache
            const int32_t nCount = (int32_t)oldData.m_pendingTextData.size();
            ...
            if (nUpdateCacheStartIndex == (size_t)-1) {
                //Append at the end; the line number (physical line number, logical row number) does not need updating because the correct values were set during incremental drawing
    ...
    //Fix the physical line number, logical row number, and the drawn destination area values of this line
    ...
    for (int32_t nIndex = 0; nIndex < nCount; ++nIndex) {
        ...
        if (!bUpdateLineRows && bUpdateIndex) {
            if ((nUpdateCacheStartIndex != (size_t)-1)) {
                //Update the line number (there are modifications and the modification point is not at the end)
                if (nIndex >= (int32_t)nUpdateCacheStartIndex) {
                    bUpdateLineRows = true;
                }
            }
            else if (!bAppendUpdateAtEnd) {
                //No modifications, only deletions; or the newly modified line is just an empty line
                if (pendingData.m_nLineNumber >= nStartLine) {
                    bUpdateLineRows = true;
                }
            }
        ...
        if (pendingData.m_nLineNumber >= nStartLine) {
            //The index of the corresponding data is updated to an invalid value; it will not be used afterwards
            pendingData.m_nDataIndex = (uint32_t)-1;

            //Update the drawn destination area of this line
                                         ...
                                         std::vector<std::vector<UiRect>>* pRichTextRects)
{
    ...
    for (const SharePtr<TPendingDrawRichText>& spTextData : pendingTextData) {
        const TPendingDrawRichText& textData = *spTextData;
        //When drawing through the cache, the textData.m_nDataIndex value must not be used; this value is incorrect during incremental drawing
        ...
        if ((textData.m_textView.size() == 1) && (textData.m_textView[0] == L'\t')) {
            ...
            const char* text = (const char*)textData.m_textView.data();
            size_t len = textData.m_textView.size() * textCharSize; //The number of bytes
            DrawTextString(rcDestRect, text, len, textEncoding,
                           textData.m_textStyle | DrawStringFormat::TEXT_SINGLELINE,
                           skPaint, textData.m_spFont.get());
        }
                                  ...
                                  uint32_t uFormat, const SkPaint& skPaint, IFont* pFont) const
{
    ...
    //When drawing text, do not use a clip region (may cause the text drawing to exceed the bounds)
