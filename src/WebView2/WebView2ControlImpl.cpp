...
HRESULT WebView2Control::Impl::InitializeAsync(const DString& userDataFolder, InitializeCompletedCallback callback)
{
    ...
    // Start the asynchronous initialization flow
        ...
        ui::ComCallback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler,
            ...
            [this](HRESULT result, ICoreWebView2Controller* controller) -> HRESULT {
                ...
                if (SUCCEEDED(result) && (controller != nullptr)) {
                    ...
                    // Get the WebView instance - use a synchronous call
...
void WebView2Control::Impl::OnInitializationCompleted(HRESULT result)
{
    ...
    // Initialize the focus change response event
    InitializeFocusChanged();    

    // Initialize the shortcut key events: support for F11/F12
    InitializeAcceleratorKeyPressed();

    // Initialize the position of the control
    ...
    // The initial URL; navigate to this URL
    ...
    // Call the initialization completed callback
    auto callback = std::move(m_initializeCompletedCallback);
    if (callback) {
        callback(result);
    }
...
void WebView2Control::Impl::InitializeSettings()
{
    ...
    // Initialize the settings - a synchronous operation, but wrapped in the asynchronous flow
    ...
    HRESULT hr = m_spWebView2Controller->add_GotFocus(
        ...
        IID_ICoreWebView2FocusChangedEventHandler>(
            [this](ICoreWebView2Controller* sender, IUnknown* /*args*/) -> HRESULT {
                ...
                // When the page gains focus, synchronously set the focus of the associated control
                if (m_pControl != nullptr) {
                    m_pControl->SetFocus();
                }
                return S_OK;
    ...
    HRESULT hr = m_spWebView2Controller->add_AcceleratorKeyPressed(
        ...
        IID_ICoreWebView2AcceleratorKeyPressedEventHandler>(
            [this](ICoreWebView2Controller* sender, ICoreWebView2AcceleratorKeyPressedEventArgs* args) -> HRESULT {
                ...
                // Page shortcut key event: get the virtual key code of the pressed shortcut key
                ...
                if (keyKind == COREWEBVIEW2_KEY_EVENT_KIND_KEY_DOWN && virtualKey == VK_ESCAPE) {
                    ...
                    if (pWindow->IsWindowFullscreen()) {
                        ...
                        // Prevent WebView2 from passing the event to the web page, so that the web page does not handle it itself
                    ...
                    if (IsEnableF11()) {
                        ...
                        // Execute the fullscreen toggle logic
                        if (pWindow->IsWindowFullscreen() && (pWindow->GetFullscreenControl() == m_pControl)) {
                            ...
                            // Enter the page fullscreen state
                    ...
                    if (!IsEnableF12()) {
                        // Set F12 as not supported; intercept the F12 shortcut key
                        args->put_Handled(TRUE);
                    }
                }
                return S_OK;
...
void WebView2Control::Impl::AddNavigationStateChangedCallback()
{
    ...
    // Register the navigation starting event
    ...
    // Register the navigation completed event
        ...
        HRESULT hr = m_spWebView2->add_NewWindowRequested(
            ui::ComCallback<ICoreWebView2NewWindowRequestedEventHandler,
                ...
                [this](ICoreWebView2* sender, ICoreWebView2NewWindowRequestedEventArgs* args) -> HRESULT {
                    ...
                    // The old API has no TargetFrameName; use an empty string instead
                    bool bAllow = false;
                    if (m_newWindowRequestedCallback) {
                        ...
                        if (bUserInitiated) {
                            // Manual click; allow the popup window
                            bAllow = true;
                        }
                    }

                    if (bAllow) {
                        // Overwrite the current page (opening a new window or creating a new tab is not supported for now)
...
HRESULT WebView2Control::Impl::SetHistoryChangedCallback(HistoryChangedCallback callback)
{
    ...
    // If already initialized and there is a callback, register the event
    if (callback && m_spWebView2) {
                                    ...
                                    int32_t& nWidth, int32_t& nHeight, std::vector<uint8_t>& imageData)
{
    ...
    if (nWindowScaleFactor > 0) {
        // Scale the image size according to the DPI
...
// Download the website icon (favicon)
...
bool WebView2Control::Impl::DownloadFavIconImage()
{
    ...
    GlobalManager::Instance().Thread().PostTask(nThreadIdentifier, m_pControl->ToWeakCallback([this, strUrl]() {
            // Switch to the child thread to download the icon
            ...
            if (DownloadFaviconToVector(strUrl.c_str(), iconData)) {
                ...
                if ((m_pControl != nullptr) && (m_favIconChangedCallback != nullptr)) {
                    GlobalManager::Instance().Thread().PostTask(ui::kThreadUI,
                        m_pControl->ToWeakCallback([this, nWidth, nHeight, imageData]() {
                                // Switch to the UI thread to execute the callback function
                ...
                // The icon download failed; the callback needs to be invoked
                if ((m_pControl != nullptr) && (m_favIconChangedCallback != nullptr)) {
                    GlobalManager::Instance().Thread().PostTask(ui::kThreadUI,
