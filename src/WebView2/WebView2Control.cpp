...
static bool CaptureWindowBitmap_Win32(HWND hwnd, RECT rect, std::vector<uint8_t>& bitmap, int32_t& width, int32_t& height)
{
    ...
    bi.biWidth = width;
    bi.biHeight = -height;  // positive means bottom-up, negative means top-down
    bi.biPlanes = 1;
    bi.biBitCount = 32;
...
/** Captures the preview image of the web page (only the display area of the control, not the entire web page)
*/
