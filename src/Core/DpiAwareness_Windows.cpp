...
{
    if (dpiAwarenessMode == DpiAwarenessMode::kFromManifest) {
        //Defined as reading the DPI awareness feature from the exe's manifest, no code setting required
        return dpiAwarenessMode;
    }
    if (!::IsWindowsVistaOrGreater()) {
        //Systems below Vista do not support DPI awareness
        m_dpiAwarenessMode = DpiAwarenessMode::kDpiUnaware;
        return DpiAwarenessMode::kDpiUnaware;
    }

    //Note: if DPI awareness is set in the application (.exe) manifest, the related setting APIs will fail to be called
    //     If this function has been called once before, the second call will fail (there should be a restriction, only allowing it to be set once)
    if (dpiAwarenessMode != DpiAwarenessMode::kDpiUnaware) {
        bool bSetOk = false;
