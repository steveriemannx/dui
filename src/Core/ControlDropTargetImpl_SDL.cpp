...
void ControlDropTargetImpl_SDL::OnDropTexts(const std::vector<U8String>& textList, const UiPoint& pt)
{
    if (m_pControl != nullptr) {
        ControlDropData_SDL data;
        data.m_bHandled = false;
...
void ControlDropTargetImpl_SDL::OnDropFiles(const U8String& source, const std::vector<U8String>& fileList, const UiPoint& pt)
{
    if (m_pControl != nullptr) {
        if (!fileList.empty()) {
            ...
            U8String fileTypes = m_pControl->GetDropFileTypes();
