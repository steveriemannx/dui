...
static SDL_Cursor* LoadCursorFromData(const Window* pWindow, std::vector<uint8_t>& fileData, const FilePath& imageFilePath)
{
    ...
    int hot_x = 0;
    int hot_y = 0;
    U8String imagePath = imageFilePath.ToString();
    size_t nDot = imagePath.rfind('.');
    if ((nDot != U8String::npos) && (fileData.size() > 16)) {
        U8String ext = imagePath.substr(nDot);
        if (StringUtil::IsEqualNoCase(ext, (".cur"))) {
