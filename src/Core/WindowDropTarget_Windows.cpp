...
HRESULT WindowDropTarget::DragEnter(IDataObject* pDataObj, DWORD grfKeyState, POINTL pt, DWORD* pdwEffect)
{
    ...
    // Process this event in the window first
    ...
    // Process this event in the control
    OnDragOver(grfKeyState, pt, pdwEffect);
    // S_OK must be returned, otherwise this drag and drop is terminated and the drag and drop operation cannot continue
    return S_OK;
}
