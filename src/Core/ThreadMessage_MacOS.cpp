...
bool ThreadMessage::PostMsg(uint32_t msgId, WPARAM wParam, LPARAM lParam, uint32_t* nErrorCode)
{
    ...
    if (msgId == m_impl->m_msgId) {
        bRet = MessageLoop_MacOS::PostUserEvent(msgId, wParam, lParam);
    }
    return bRet;
}
...
void ThreadMessage::RemoveDuplicateMsg(uint32_t msgId)
{
    ...
    ASSERT(msgId == m_impl->m_msgId);
    if (msgId == m_impl->m_msgId) {
        MessageLoop_MacOS::RemoveDuplicateMsg(msgId);
    }
}
...
{
    if (m_impl->m_msgId != 0) {
        MessageLoop_MacOS::RemoveUserMessageCallback(m_impl->m_msgId);
    }
    m_impl->m_msgId = msgId;
    if (m_impl->m_msgId != 0) {
        MessageLoop_MacOS::AddUserMessageCallback(m_impl->m_msgId, callback);
    }
}
