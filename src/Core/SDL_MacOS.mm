...
bool ModifyNsWindowShadowType(void* pNSWindow, NativeWindowShadowType nativeShadowType)
{
    ...
    case NativeWindowShadowType::kShadowSystemDisabled:
        [window setHasShadow:NO];
        // Take the titled style mask back off. Turning it *on* is what gives the
        // window the OS rounded corners, and leaving it on when the theme asks for
        // no shadow keeps those corners (and clips a drawn border at them) -- the
        // mask has to be removed here, not just the layer radius cleared.
