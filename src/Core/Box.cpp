...
void Box::PaintChild(IRender* pRender, const UiRect& rcPaint)
{
    ...
    for (auto pControl : m_items) {
        ...
        if (pControl->GetPaintOrder() != 0) {
            // Set the paint order, add to the delayed paint list
    ...
    if (!delayItems.empty()) {
        ...
        // Paint the delayed paint controls
...
UiEstSize Box::EstimateSize(UiSize szAvailable)
{
    ...
    // The size of the Box control itself, not including the outer margin (it is also a control, can have text/background images, etc.)
    ...
    // The size of the child controls, including the inner padding, but not the outer margin
...
{
    // Match and find in child controls and itself
...
{
    // ptMouse: coordinates already adapted to the container itself
...
bool Box::DoAddItemAt(Control* pControl, size_t iIndex)
{
    ...
    // Synchronize the Enable and Visible properties
    pControl->SetAncestorEnabled(IsEnabled());
    pControl->SetAncestorVisible(IsVisible());

    // Call the initialization function after being added to the parent container
    pControl->Init();
    if (IsVisible()) {
        Arrange();
    }    
...
{
    return UIFLAG_DEFAULT; // Box does not support TAB focus switching by default
}
