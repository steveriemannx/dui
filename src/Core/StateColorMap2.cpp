...
void StateColorMap2::SetStateColor(ControlStateType stateType, const std::string& color)
{
...
std::string StateColorMap2::GetStateColor(ControlStateType stateType) const
{
    size_t nIndex = (size_t)stateType;
    if (nIndex < m_stateColors.size()) {
    ...
    if (m_pControl != nullptr) {        
        if (m_pControl->IsAnimationPlayerPlaying(AnimationType::kAnimationHot)) {
            if ((stateType == kControlStateNormal || stateType == kControlStateHot) && HasStateColor(kControlStateHot)) {
                ...
                // Draw the color of the Hot state (semi-transparent)
...
void StateColorMap2::DoPaintStateColor(IRender* pRender, UiRect rcPaint, ControlStateType stateType, UiColor colorValue, uint8_t nAlpha) const
{
    ...
    if (!rcMargin.IsEmpty()) {
        rcPaint.Deflate(rcMargin);// Remove the outer margin
        if (rcPaint.IsEmpty()) {
            return;
        }
    }
