    ...
    case SDL_EVENT_TEXT_INPUT:
        {
            ...
            if (sdlEvent.text.text != nullptr) {
                ...
                std::wstring textW = StringConvert::UTF8ToWString(sdlEvent.text.text);
                if (!textW.empty()) {
...
bool NativeWindow_SDL::CreateWindowAndRender(NativeWindow_SDL* pParentWindow, const WindowCreateAttributes& createAttributes)
{
    ...
    m_sdlRenderer = CreateSdlRenderer(createAttributes.m_nativeRenderName);
    ASSERT(m_sdlRenderer != nullptr);
    if (m_sdlRenderer == nullptr) {
...
SDL_Window* NativeWindow_SDL::CreateSdlWindow(NativeWindow_SDL* pParentWindow, const WindowCreateAttributes& createAttributes)
{
    ...
    QueryRenderProperties(createAttributes.m_nativeRenderName, bOpenGL, bOpenGLES2, bSupportTransparent);
#endif
...
#ifndef DUI_BUILD_FOR_WIN
    //Linux platform
    std::vector<U8String> renderNames;
    GetRenderNameList(createAttributes.m_nativeRenderName, renderNames);
    U8String firstRenderName;
    if (!renderNames.empty()) {
        firstRenderName = renderNames.front();
    }
    if (bSupportTransparent && (firstRenderName == ("opengles2"))) {
...
bool NativeWindow_SDL::CreateChildWnd(NativeWindow_SDL* pParentWindow, int32_t nX, int32_t nY, int32_t nWidth, int32_t nHeight)
{
    ...
    if (hParent != nullptr) {
        const U8String className = ("dui_child_window");
        WNDCLASS wc = { 0 };
...
SDL_Renderer* NativeWindow_SDL::CreateSdlRenderer(const U8String& sdlRenderName) const
{
    std::vector<U8String> renderNames;
    ...
    for (auto iter = renderNames.begin(); iter != renderNames.end(); ++iter) {
        const U8String renderName = StringConvert::TToUTF8(*iter);
...
void NativeWindow_SDL::GetRenderNameList(const U8String& externalRenderName, std::vector<U8String>& renderNames) const
{
            ...
            for (int32_t nWindow = 0; nWindow < nWindowCount; ++nWindow) {
                ...
                if (pSdlRender != nullptr) {
                    ...
                    if ((pRenderName != nullptr) && (pRenderName[0] != '\0')) {
                        ...
                        U8String name = StringConvert::UTF8ToT(std::string(pRenderName));
    ...
    if (!externalRenderName.empty()) {
        std::list<U8String> renderNameList = StringUtil::Split(externalRenderName, (","));
    ...
    renderNames.push_back(("direct3d11"));
    renderNames.push_back(("opengl"));
    renderNames.push_back(("vulkan"));
#else
    //Linux platform: currently, those supporting transparency (attribute: SDL_WINDOW_TRANSPARENT) are: "opengles2", "vulkan"
    renderNames.push_back(("opengles2"));
    renderNames.push_back(("vulkan"));
    renderNames.push_back(("opengl"));
#endif

    //Remove those that do not exist
    std::vector<U8String> renderDrivers;
    ...
    for (int32_t nRenderIndex = 0; nRenderIndex < nRenderCount; ++nRenderIndex) {
        ...
        if (renderName != nullptr) {
            U8String name = StringConvert::UTF8ToT(std::string(renderName));
            if (!name.empty()) {
...
void NativeWindow_SDL::QueryRenderProperties(const U8String& externalRenderName, bool& bOpenGL, bool& bOpenGLES2, bool& bSupportTransparent) const
{
    bOpenGL = false;
    bOpenGLES2 = false;
...
bool NativeWindow_SDL::IsRenderSupportTransparent(const U8String& renderName) const
{
    bool bSupportTransparent = false;
#ifdef DUI_BUILD_FOR_WIN
    if (renderName == ("direct3d11")) {
        bSupportTransparent = true;
    }
    else if (renderName == ("opengl")) {
        bSupportTransparent = true;
    }
    else if (renderName == ("vulkan")) {
        bSupportTransparent = true;
    }
#else
    if (renderName == ("opengles2")) {
        bSupportTransparent = true;
    }
    else if (renderName == ("opengl")) {
        bSupportTransparent = true;
...
U8String NativeWindow_SDL::GetVideoDriverName() const
{
    U8String driverName;
...
U8String NativeWindow_SDL::GetWindowRenderName() const
{
    U8String renderName;
    if (m_sdlWindow != nullptr) {
...
bool NativeWindow_SDL::IsPopupWindow() const
{
...
{
    U8String videoDriverName = StringUtil::MakeLowerString(GetVideoDriverName());
    return videoDriverName == ("x11");
}

bool NativeWindow_SDL::IsVideoDriverWayland() const
{
    U8String videoDriverName = StringUtil::MakeLowerString(GetVideoDriverName());
    return videoDriverName == ("wayland");
}
...
bool NativeWindow_SDL::IsSystemShadowSupported() const
{
    ...
    // True: Shadow::GetSupportedShadowType() remaps self-drawn shadow types to the
    // OS shadow on macOS. A drawn one needs per-pixel window transparency, and on
    // this platform it does not composite -- the shadow margin renders as a solid
    // black border. Tried and confirmed three ways (see the project notes):
    // answering false here, clearing the render target to transparent, and forcing
    // the opengles2 renderer -- all three leave the black border.
    return true;
#else
    return false;
#endif
...
void NativeWindow_SDL::SetUseSystemCaption(bool bUseSystemCaption)
{
    ...
    bool bOpenGLES2 = false;
    bool bSupportTransparent = false;
    QueryRenderProperties((""), bHasOpenGL, bOpenGLES2, bSupportTransparent);
    if (!bSupportTransparent) {
        m_bUseSystemCaption = true;
    }
#endif
...
void NativeWindow_SDL::SetText(const U8String& strText)
{
    ASSERT(IsWindow());
    //Converts to UTF-8 encoding
    U8String utf8Text = StringConvert::TToUTF8(strText);
    bool nRet = SDL_SetWindowTitle(m_sdlWindow, utf8Text.c_str());
    ASSERT_UNUSED_VARIABLE(nRet);
}

U8String NativeWindow_SDL::GetText() const
{
    U8String windowText;
    if (m_sdlWindow != nullptr) {
...
{
    PerformanceStat statPerformance(("PaintWindow, NativeWindow_SDL::PaintWindow(Total)"));
    if (bPaintAll) {
        //Draw everything
...
bool NativeWindow_SDL::SetWindowIcon(const std::vector<uint8_t>& iconFileData, const U8String& iconFileName)
{
...
void NativeWindow_SDL::OnDropTexts(const std::vector<U8String>& textList, const UiPoint& pt, bool& bHandled)
{
    ControlDropData_SDL data;
    data.m_bHandled = false;
    data.m_ptClientX = pt.x;
...
void NativeWindow_SDL::OnDropFiles(const U8String& source, const std::vector<U8String>& fileList, const UiPoint& pt, bool& bHandled)
{
    ControlDropData_SDL data;
    data.m_bHandled = false;
    data.m_ptClientX = pt.x;
