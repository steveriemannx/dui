...
struct DirectoryTreeImpl::TImpl
{
    ...
    */
    ...
    /** Shared file icon (large icon)
    */
    uint32_t m_nLargeFileIconID = 0;

    /** Shared file icon (small icon)
    */
        ...
        for (const auto& entry : std::filesystem::directory_iterator(stdPath)) {
            ...
            if (bDirectory) {
                ...
                if (bRegularFile) {
                    ...
                    //Last modified time and file size of the file
                    struct stat buf{0, };
                    int result = ::stat(entry.path().native().c_str(), &buf);
                    if (result == 0) {
...
// Detect the physical type of the device
static DirectoryTree::DeviceType detect_device_type(const std::string& base_dev)
{
    ...
    // Check whether the device exists under /sys/block
    ...
    // USB device (check the driver link)
                                        ...
                                        std::vector<DirectoryTree::DiskInfo>& diskInfoList)
{
    ...
    while (::fgets(line, sizeof(line) - 8, fp)) {
        ...
        // Skip virtual file systems (e.g., proc, sysfs)
        ...
        if (::statvfs(mount_point, &vfs) != 0) {
            continue; // Skip mount points that cannot be accessed
        }
