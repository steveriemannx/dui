...
/** DLL module management for the RichEdit control
*/
...
public:
    ...
    */
    ...
    /** The DLL that RichEdit depends on, loaded and returns the handle
    */
...
void RichEditHost::Init()
{
    ...
    //Enabled by default: when the user presses ENTER on the last line, automatically scroll the text up one page.
    m_dwStyle |= UI_ES_AUTOVSCROLL;

    //Enabled by default: when the user types a character at the end of a line, automatically scroll the text 10 characters to the right.
...
void RichEditHost::TxInvalidateRect(LPCRECT prc, BOOL /*fMode*/)
{
    ...
    //Mark the specified area of the window as a dirty area and redraw (use the control position part, to avoid triggering redraws of other controls)
...
HRESULT RichEditHost::TxGetPropertyBits(DWORD dwMask, DWORD* pdwBits)
{
    ...
    if (m_bFlashPasswordChar) {
        //First show the character, then show the password character
    ...
    if (m_fSaveSelection) {
        //Save selection: if TRUE, the boundaries of the selection should be saved when the control is inactive.
...
{
    //When the user types a character at the end of a line, automatically scroll the text 10 characters to the right. When the user presses Enter, the control scrolls all text back to the zero position.
    if (bEnable) {
        m_dwStyle |= UI_ES_AUTOHSCROLL;
    }
    else {
...
void RichEditHost::GetControlRect(UiRect* prc)
{
    ...
    if ((m_dwStyle & UI_ES_VCENTER) || (m_dwStyle & UI_ES_BOTTOM)) {        
        ...
        if (m_dwStyle & UI_ES_VCENTER) {
            //Vertical center alignment (only applied when the text height is less than the target area height)
            ...
            //Vertical bottom alignment (only applied when the text height is less than the target area height)
