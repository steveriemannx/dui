...
void ColorPickerCustom::InitPicker()
{
    ...
    m_pRegularPicker = dynamic_cast<ColorPickerRegular*>(pWindow->FindControl(DUI_T("color_picker_custom_regular")));
    ...
    m_pSpectrumControl = dynamic_cast<ColorControl*>(pWindow->FindControl(DUI_T("color_picker_custom_spectrum")));
    ...
    m_pNewColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl(DUI_T("color_picker_new_color_edit")));
    ...
    m_rgbA.m_pColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl(DUI_T("color_picker_edit_RGB_A")));
    m_rgbA.m_pColorSlider = dynamic_cast<ColorSlider*>(pWindow->FindControl(DUI_T("color_picker_slider_RGB_A")));
    m_rgbR.m_pColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl(DUI_T("color_picker_edit_RGB_R")));
    m_rgbR.m_pColorSlider = dynamic_cast<ColorSlider*>(pWindow->FindControl(DUI_T("color_picker_slider_RGB_R")));
    m_rgbG.m_pColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl(DUI_T("color_picker_edit_RGB_G")));
    m_rgbG.m_pColorSlider = dynamic_cast<ColorSlider*>(pWindow->FindControl(DUI_T("color_picker_slider_RGB_G")));
    m_rgbB.m_pColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl(DUI_T("color_picker_edit_RGB_B")));
    m_rgbB.m_pColorSlider = dynamic_cast<ColorSlider*>(pWindow->FindControl(DUI_T("color_picker_slider_RGB_B")));
    ...
    m_hsvH.m_pColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl(DUI_T("color_picker_edit_HSV_H")));
    m_hsvH.m_pColorSlider = dynamic_cast<ColorSlider*>(pWindow->FindControl(DUI_T("color_picker_slider_HSV_H")));
    m_hsvS.m_pColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl(DUI_T("color_picker_edit_HSV_S")));
    m_hsvS.m_pColorSlider = dynamic_cast<ColorSlider*>(pWindow->FindControl(DUI_T("color_picker_slider_HSV_S")));
    m_hsvV.m_pColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl(DUI_T("color_picker_edit_HSV_V")));
    m_hsvV.m_pColorSlider = dynamic_cast<ColorSlider*>(pWindow->FindControl(DUI_T("color_picker_slider_HSV_V")));
    ...
    m_hslH.m_pColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl(DUI_T("color_picker_edit_HSL_H")));
    m_hslH.m_pColorSlider = dynamic_cast<ColorSlider*>(pWindow->FindControl(DUI_T("color_picker_slider_HSL_H")));
    m_hslS.m_pColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl(DUI_T("color_picker_edit_HSL_S")));
    m_hslS.m_pColorSlider = dynamic_cast<ColorSlider*>(pWindow->FindControl(DUI_T("color_picker_slider_HSL_S")));
    m_hslL.m_pColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl(DUI_T("color_picker_edit_HSL_L")));
    m_hslL.m_pColorSlider = dynamic_cast<ColorSlider*>(pWindow->FindControl(DUI_T("color_picker_slider_HSL_L")));
...
{
    if ((colorText.size() == 9) && (colorText.front() == DUI_T('#'))) {
...
void ColorPickerCustom::OnColorChanged(WPARAM wParam, LPARAM lParam, ChangeReason reason)
{
    ...
    if (reason != ChangeReason::NewColorEdit) {
        if (m_pNewColorEdit != nullptr) {
            DString strColor = StringUtil::Printf(DUI_T("#%02X%02X%02X%02X"),
...
void ColorPickerCustom::InitRGB(const ColorUI& colorUI, ChangeReason reason)
{
    ...
    if (colorUI.m_pColorEdit != nullptr) {
        colorUI.m_pColorEdit->SetTextNoEvent(DUI_T(""));
        colorUI.m_pColorEdit->AttachTextChanged([this, pRichEdit, pColorSlider, reason](const ui::EventArgs& /*args*/) {
                if (pRichEdit != nullptr) {
                    ...
                    if (nValue < 0) {
                        nValue = 0;
                        pRichEdit->SetTextNoEvent(DUI_T("0"));
                    }
                    else if (nValue > 255) {
                        nValue = 255;
                        pRichEdit->SetTextNoEvent(DUI_T("255"));
                    }
    ...
    if (colorUI.m_pColorSlider != nullptr) {
        ...
        colorUI.m_pColorSlider->AttachValueChanged([this, pRichEdit, reason](const ui::EventArgs& args) {
                ...
                if (pRichEdit != nullptr) {
                    DString text = StringUtil::Printf(DUI_T("%d"), value);
                ...
                if (pRichEdit != nullptr) {
                    ...
                    if (nValue < 0) {
                        ...
                        if (reason == ChangeReason::ColorHSV_H) {
                            if (nValue > 359) {
                                nValue = 359;
                                pRichEdit->SetTextNoEvent(DUI_T("359"));
                            }
                        }
                        else {
                            if (nValue > 100) {
                                nValue = 100;
                                pRichEdit->SetTextNoEvent(DUI_T("100"));
                            }
                        }
                    }
                    if (pColorSlider != nullptr) {
...
void ColorPickerCustom::UpdateRGB(const ColorUI& colorUI, const UiColor& color, int32_t flag)
{
    ...
    if (colorUI.m_pColorEdit != nullptr) {
        DString text = StringUtil::Printf(DUI_T("%d"), colorValue);
...
void ColorPickerCustom::UpdateHSV(const ColorUI& colorUIH, const ColorUI& colorUIS, const ColorUI& colorUIV, const UiColor& color, ChangeReason reason)
{
    ...
    if (needUpdate) {
        if (colorUIH.m_pColorEdit != nullptr) {
            DString text = StringUtil::Printf(DUI_T("%d"), (int32_t)hue);
    ...
    if (needUpdate) {
        if (colorUIS.m_pColorEdit != nullptr) {
            DString text = StringUtil::Printf(DUI_T("%d"), (int32_t)(sat * 100));
    ...
    if (needUpdate) {
        if (colorUIV.m_pColorEdit != nullptr) {
            DString text = StringUtil::Printf(DUI_T("%d"), (int32_t)(value * 100));
...
void ColorPickerCustom::UpdateHSL(const ColorUI& colorUIH, const ColorUI& colorUIS, const ColorUI& colorUIL, const UiColor& color, ChangeReason reason)
{
    ...
    if (needUpdate) {
        if (colorUIL.m_pColorEdit != nullptr) {
            DString text = StringUtil::Printf(DUI_T("%d"), (int32_t)(lightness * 100));
