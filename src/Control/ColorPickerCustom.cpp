...
std::string ColorPickerCustom::GetType() const { return DUI_CTR_COLOR_PICKER_CUSTOM; }
...
void ColorPickerCustom::InitPicker()
{
    ...
    m_pRegularPicker = dynamic_cast<ColorPickerRegular*>(pWindow->FindControl("color_picker_custom_regular"));
    if (m_pRegularPicker != nullptr) {
        m_pRegularPicker->AttachSelectColor([this](const ui::EventArgs& args) {
            //Forward the event to the upper layer
            OnColorChanged(args.wParam, args.lParam, ChangeReason::ColorRegular);
            return true;
            });
    }

    m_pSpectrumControl = dynamic_cast<ColorControl*>(pWindow->FindControl("color_picker_custom_spectrum"));
    ...
    m_pNewColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl("color_picker_new_color_edit"));
    if (m_pNewColorEdit != nullptr) {
        m_pNewColorEdit->AttachTextChanged([this](const ui::EventArgs& /*args*/) {
            //Forward the event to the upper layer
            std::string colorText = m_pNewColorEdit->GetText();
    ...
    m_rgbA.m_pColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl("color_picker_edit_RGB_A"));
    m_rgbA.m_pColorSlider = dynamic_cast<ColorSlider*>(pWindow->FindControl("color_picker_slider_RGB_A"));
    m_rgbR.m_pColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl("color_picker_edit_RGB_R"));
    m_rgbR.m_pColorSlider = dynamic_cast<ColorSlider*>(pWindow->FindControl("color_picker_slider_RGB_R"));
    m_rgbG.m_pColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl("color_picker_edit_RGB_G"));
    m_rgbG.m_pColorSlider = dynamic_cast<ColorSlider*>(pWindow->FindControl("color_picker_slider_RGB_G"));
    m_rgbB.m_pColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl("color_picker_edit_RGB_B"));
    m_rgbB.m_pColorSlider = dynamic_cast<ColorSlider*>(pWindow->FindControl("color_picker_slider_RGB_B"));
    ...
    m_hsvH.m_pColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl("color_picker_edit_HSV_H"));
    m_hsvH.m_pColorSlider = dynamic_cast<ColorSlider*>(pWindow->FindControl("color_picker_slider_HSV_H"));
    m_hsvS.m_pColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl("color_picker_edit_HSV_S"));
    m_hsvS.m_pColorSlider = dynamic_cast<ColorSlider*>(pWindow->FindControl("color_picker_slider_HSV_S"));
    m_hsvV.m_pColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl("color_picker_edit_HSV_V"));
    m_hsvV.m_pColorSlider = dynamic_cast<ColorSlider*>(pWindow->FindControl("color_picker_slider_HSV_V"));
    ...
    m_hslH.m_pColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl("color_picker_edit_HSL_H"));
    m_hslH.m_pColorSlider = dynamic_cast<ColorSlider*>(pWindow->FindControl("color_picker_slider_HSL_H"));
    m_hslS.m_pColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl("color_picker_edit_HSL_S"));
    m_hslS.m_pColorSlider = dynamic_cast<ColorSlider*>(pWindow->FindControl("color_picker_slider_HSL_S"));
    m_hslL.m_pColorEdit = dynamic_cast<RichEdit*>(pWindow->FindControl("color_picker_edit_HSL_L"));
    m_hslL.m_pColorSlider = dynamic_cast<ColorSlider*>(pWindow->FindControl("color_picker_slider_HSL_L"));
...
bool ColorPickerCustom::IsValidColorString(const std::string& colorText) const
{
    if ((colorText.size() == 9) && (colorText.front() == '#')) {
        for (size_t i = 1; i < colorText.size(); ++i) {
            std::string::value_type ch = colorText.at(i);
...
void ColorPickerCustom::OnColorChanged(WPARAM wParam, LPARAM lParam, ChangeReason reason)
{
    ...
    if (reason != ChangeReason::NewColorEdit) {
        if (m_pNewColorEdit != nullptr) {
            std::string strColor = StringUtil::Printf("#%02X%02X%02X%02X",
...
void ColorPickerCustom::InitRGB(const ColorUI& colorUI, ChangeReason reason)
{
    ...
    if (colorUI.m_pColorSlider != nullptr) {
        ...
        colorUI.m_pColorSlider->AttachValueChanged([this, pRichEdit, reason](const ui::EventArgs& args) {
                ...
                if (pRichEdit != nullptr) {
                    std::string text = StringUtil::Printf("%d", value);
                    if (pRichEdit->GetText() != text) {
                        pRichEdit->SetTextNoEvent(text);
                    }
                }
                //The slider changed, trigger the RGB color change event
                OnRGBChanged(reason);
                return true;
            });
    }
...
void ColorPickerCustom::InitHSV(const ColorUI& colorUI, int32_t maxValue, ChangeReason reason)
{
                ...
                if (pRichEdit != nullptr) {
                    ...
                    if (nValue < 0) {
                        ...
                        if (reason == ChangeReason::ColorHSV_H) {
                            if (nValue > 359) {
                                nValue = 359;
                                pRichEdit->SetTextNoEvent("359");
                            }
                        }
                        else {
                            if (nValue > 100) {
                                nValue = 100;
                                pRichEdit->SetTextNoEvent("100");
                            }
                        }
                    }
                    if (pColorSlider != nullptr) {
    ...
    if (colorUI.m_pColorSlider != nullptr) {
        ...
        colorUI.m_pColorSlider->AttachValueChanged([this, pRichEdit, reason](const ui::EventArgs& args) {
            ...
            //Trigger the HSV color change event
            OnHSVChanged(reason);
            return true;
            });
    }
...
void ColorPickerCustom::InitHSL(const ColorUI& colorUI, int32_t maxValue, ChangeReason reason)
{
    ...
    if (colorUI.m_pColorSlider != nullptr) {
        ...
        colorUI.m_pColorSlider->AttachValueChanged([this, pRichEdit, reason](const ui::EventArgs& args) {
            ...
            //Trigger the HSL color change event
            OnHSLChanged(reason);
            return true;
            });
    }
...
void ColorPickerCustom::UpdateRGB(const ColorUI& colorUI, const UiColor& color, int32_t flag)
{
    ...
    if (colorUI.m_pColorEdit != nullptr) {
        std::string text = StringUtil::Printf("%d", colorValue);
...
void ColorPickerCustom::UpdateHSV(const ColorUI& colorUIH, const ColorUI& colorUIS, const ColorUI& colorUIV, const UiColor& color, ChangeReason reason)
{
    ...
    if ((reason == ChangeReason::ColorHSV_H) ||
        ...
        //Get the color value from the control
    ...
    if (needUpdate) {
        if (colorUIH.m_pColorEdit != nullptr) {
            std::string text = StringUtil::Printf("%d", (int32_t)hue);
    ...
    if (needUpdate) {
        if (colorUIS.m_pColorEdit != nullptr) {
            std::string text = StringUtil::Printf("%d", (int32_t)(sat * 100));
    ...
    if (needUpdate) {
        if (colorUIV.m_pColorEdit != nullptr) {
            std::string text = StringUtil::Printf("%d", (int32_t)(value * 100));
...
void ColorPickerCustom::UpdateHSL(const ColorUI& colorUIH, const ColorUI& colorUIS, const ColorUI& colorUIL, const UiColor& color, ChangeReason reason)
{
    ...
    if (needUpdate) {
        if (colorUIL.m_pColorEdit != nullptr) {
            std::string text = StringUtil::Printf("%d", (int32_t)(lightness * 100));
