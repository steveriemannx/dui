...
void TabCtrl::OnInit()
{
    ...
    //Adjust the display state of the tab separator lines
...
void TabCtrl::HandleEvent(const EventArgs& msg)
{
    if ((msg.GetSender() == this) && (msg.eventType == kEventSelect)) {
        //Try to set the associated TabBox
...
void TabCtrl::AdjustItemLineStatus()
{
    ...
    for (size_t nItem = 0; nItem < nCount; ++nItem) {
        ...
        if ((state == kControlStateHot) || (state == kControlStatePushed) || pItem->IsSelected()) {
            //Active tab: do not show the separator line
            pItem->SetItemLineVisible(false);

            if (pLastItem != nullptr) {
                //The tab before the active tab: do not show the separator line
                pLastItem->SetItemLineVisible(false);
            }
        }
        else {
            //Tabs in other states: show the separator line
            pItem->SetItemLineVisible(true);
        }
        
        if (nItem == (nCount - 1)) {
            //The last tab: do not show the separator line
            pItem->SetItemLineVisible(false);
        }

        //Record the previous tab
        pLastItem = pItem;
    }
}
...
void TabCtrlItem::HandleEvent(const EventArgs& msg)
{
    ...
    if ((msg.GetSender() == this) && (m_pLine != nullptr) && (msg.eventType == kEventStateChanged)) {
        //Handle the state of the separator line
        AdjustItemLineStatus();
    }
}
...
bool TabCtrlItem::ButtonDown(const EventArgs& msg)
{
    ...
    if (!IsSelected() && IsActivatable()) {
        //Select when the mouse left button is pressed
...
{
    //Draw the shape of the tab page (expressed through colors)
    if (IsSelected()) {
...
void TabCtrlItem::AddTabItemPath(IPath* path, const UiRect& rect, UiSize roundSize) const
{
    ...
    //Ensure both the width and height of the corner radius are even
    if ((roundSize.cx % 2) != 0) {
        roundSize.cx += 1;
    }
    if ((roundSize.cy % 2) != 0) {
...
bool TabCtrlItem::IsEnableDragOrder() const
{
    ...
    if (pTabCtrl != nullptr) {
        if (!pTabCtrl->IsEnableDragOrder()) {
            //The master switch of TabCtrl, which turns off the overall feature
            return false;
        }
    }
