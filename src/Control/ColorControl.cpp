...
void ColorControl::PaintBkImage(IRender* pRender)
{
    ...
    //Draw the indicator of the selected position
...
{        
    ...
    if (m_spBitmap != nullptr) {
        if (((int32_t)m_spBitmap->GetWidth() == nWidth) &&
            ...
            //The width and height have not changed, no need to regenerate
    ...
    if (m_spBitmap != nullptr) {            
        ...
        if (pPixelBits != nullptr) {
            ...
            double satStep = 1.0 / (nHeight - 1); //The increment of sat for each additional row
            double sat = 1.0;//The first value is 1.0
            for (int32_t nRow = 0; nRow < nHeight; ++nRow) {                    
                ColorConvert::HSV_HUE(pData, nWidth, sat, 1.0);
                pData += nWidth;
                sat -= satStep;
                if ((sat < 0.0) || (nRow == (nHeight - 2))){
                    //The last value is 0.0
                    sat = 0.0;
                }
            }
...
void ColorControl::OnSelectPosChanged(const UiRect& rect, const UiPoint& pt)
{
    ...
    if (m_spBitmap != nullptr) {
        ...
        int32_t colorXY = nWidth * nRow + nColumn; //The coordinate position of the point of the color
