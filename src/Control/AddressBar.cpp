...
{
    BaseClass::SetPos(rc);
    //Update the status of child controls
    UpdateAddressBarControlsStatus();
}
...
void AddressBar::OnInit()
{
    ...
    //Basic structure of the address bar
    //<AddressBar>
    //   <RichEdit/>
    //   <HBox>
    //        <HBox>
    ...
    //When focus is obtained, switch to the edit control
...
{
    //If the display width is insufficient, some controls need to be hidden
    if (m_pBarBox == nullptr) {
        return;
    }
    UiRect rc = GetPos();
    ...
    int32_t nKeepWidth = 0; //Width that needs to be reserved
    ...
    std::vector<int32_t> originalWidths; //Actual width occupied by each control
    int32_t nPathWidth = nKeepWidth;     //Total width occupied by the path controls
    for (Control* pPathButton : pathButtonList) {
        rcPadding = pPathButton->GetPadding();
        nPathWidth -= (rcPadding.left + rcPadding.right);//Because the result returned by EstimateSize already includes the padding, it needs to be subtracted to avoid double counting
    ...
    if ((nPathWidth > nBarWidth) && (pathButtonList.size() > 1) && (originalWidths.size() == pathButtonList.size())) {
        //First control: the root directory control stays unchanged
        ...
        //Starting from the second control, dynamically adjust the width proportionally
        int32_t totalWidth = nBarWidth - nKeepWidth; //Total width available for the path controls
...
std::vector<int32_t> AddressBar::AdjustControlsWidth(const std::vector<int32_t>& originalWidths, int32_t totalWidth)
{
    ...
    const int32_t nMinWidth = 2; //Minimum width
    if (totalWidth < (int32_t)adjustedWidths.size() * nMinWidth) {
        //Insufficient controls; set to the minimum width
    ...
    // Find the controls that need adjustment (those larger than the average width)
    ...
    // Calculate the total width that needs to be reduced from the larger controls
    int32_t reduceTotal = std::accumulate(originalWidths.begin(), originalWidths.end(), 0) - totalWidth;
    if (reduceTotal <= 0) {
        return originalWidths;  // No reduction needed
    }

    // Calculate the original total width of the larger controls
    int32_t largeSum = 0;
    ...
    for (int32_t& v : adjustedWidths) {
        //Limit the minimum value to avoid 0 or negative values
        if (v < nMinWidth) {
            v = nMinWidth;
        }
    }
