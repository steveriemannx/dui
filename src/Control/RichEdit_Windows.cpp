...
/** Implementation of the drag-and-drop operation interface (drop-only operation)
*/
...
public:
    ...
    virtual int32_t DragEnter(void* pDataObj, uint32_t grfKeyState, const UiPoint& pt, uint32_t* pdwEffect) override
    {
        ...
        //Record the selection and scrollbar position (restore the original state when the operation is cancelled)
        ...
        if ((m_pRichEdit != nullptr) && !m_dropFileList.empty()){
            ...
            //File drag-and-drop is supported, return directly
            return S_OK;
        }
    ...
    virtual int32_t DragOver(uint32_t grfKeyState, const UiPoint& pt, uint32_t* pdwEffect) override
    {
        ...
        if ((m_pRichEdit != nullptr) && !m_dropTextList.empty()) {
            //Drop text operation: validate the text
            if (!m_pRichEdit->IsMultiLine()) {
                if (m_dropTextList.size() > 1) {
                    //Dropped multi-line text is invalid
                    return hr;
                }
            }
                ...
                if (!dropText.empty()) {
                    ...
                    for (size_t index = 0; index < count; ++index) {
                        ...
                        if ((dropText[index] > L'9') || (dropText[index] < L'0')) {
                            //There are non-digit characters; dropping is forbidden
                            return hr;
                        }
                    }
                }
            ...
            if (!limitChars.empty()) {
                ...
                for (const DString& dropText: m_dropTextList) {
                    ...
                    for (size_t index = 0; index < count; ++index) {
                        ...
                        if (!bMatch) {
                            //There are invalid characters; dropping is forbidden
                            return hr;
                        }
                    }
                }
        ...
        if (SUCCEEDED(txResult) && (pDropTarget != nullptr)) {
            ...
            if ((hr == S_OK) && (dwEffect != DROPEFFECT_NONE)) {
                //On success, set the caret of the current RichEdit control to the mouse position so the drop target position can be seen
                if (m_pRichEdit != nullptr) {
                    UiPoint clientPt = pt;
                    m_pRichEdit->ScreenToClient(clientPt);
                    if (!m_pRichEdit->IsFocused()) {
                        //Must be set as the focused control, otherwise CharFromPos will fail
    ...
    virtual int32_t DragLeave(void) override
    {
        ...
        //Restore the original state when dragging leaves
        if (m_pRichEdit != nullptr) {
...
{
    //Create the RichEditHost interface
    ...
    //Initialize whether the selection is hidden
...
void RichEdit::SetAttribute(const DString& strName, const DString& strValue)
{
    if (strName == _T("vscrollbar")) {
        ...
        // Example: "0,0" disables zoom, "2,1" zooms to 200%, "1,2" zooms to 50% 
        UiSize zoomValue;
...
UiFont RichEdit::GetFontInfo() const
{
    ...
    if (cf.dwMask & CFM_SIZE) {
        ...
        //In rich text mode, if the selection is mixed, the font size information may be missing
        uiFont.m_fontSize = 0;
    }
    ...
    if (cf.dwMask & CFM_FACE) {
        ...
        //In rich text mode, if the selection is mixed, the font name information may be missing
...
void RichEdit::OnTxNotify(DWORD iNotify, void *pv)
{
    ...
    case EN_LINK:   
        ...
        break;
    case EN_CHANGE:
        //The text content has changed, send the event
        OnTextChanged();            
        break;
    case EN_SELCHANGE:
        //Selection changed
...
// A multiline non-rich-format richedit has a scrollbar bug: when the last line is empty, LineDown and SetScrollPos cannot scroll to the end
...
UiEstSize RichEdit::EstimateSize(UiSize /*szAvailable*/)
{
    ...
    if (fixexSize.cx.IsAuto() || fixexSize.cy.IsAuto()) {
        ...
        //The returned size must include the padding
...
void RichEdit::SetPos(UiRect rc)
{
    ...
    if (m_pRichHost != nullptr) {
        //Adjust the position of the edit box, subtract the text padding
        UiRect textRect = rc;
        UiPadding rcTextPadding = GetTextPadding();
    ...
    if ((pVScrollBar != nullptr) && pVScrollBar->IsValid()) {        
        int32_t nHScrollbarHeight = 0; //Height of the horizontal scrollbar
        UiRect rcScrollBarPos(rc.right, rc.top, rc.right + pVScrollBar->GetFixedWidth().GetInt32(), rc.bottom);
        if ((pHScrollBar != nullptr) && pHScrollBar->IsValid()) {
            //The bottom of the vertical scrollbar needs to reach the bottom of the container
...
{
    //Use the arrangement method of the default layout
...
void RichEdit::HandleEvent(const EventArgs& msg)
{
    ...
    if (msg.eventType == kEventMouseWheel) {
        if (IsKeyDown(msg, ModifierKey::kControl) && IsEnableWheelZoom()) {
            //Ctrl + mouse wheel: zoom function
...
bool RichEdit::OnSetCursor(const EventArgs& msg)
{
    if (m_bUseControlCursor) {
        //Use the cursor set by the Control
        return BaseClass::OnSetCursor(msg);
    }

    if (!IsEnabled()) {
        //When not enabled, use the default cursor
        return BaseClass::OnSetCursor(msg);
    }
...
bool RichEdit::OnSetFocus(const EventArgs& /*msg*/)
{
    ...
    //Open the input method editor when gaining focus
    ...
    //Ensure the selection state is shown when gaining focus, otherwise the selected text state cannot be seen properly
...
bool RichEdit::OnKillFocus(const EventArgs& msg)
{
    ...
    if (!m_bContextMenuShown) {
        //When losing focus, synchronize the selection text state
        ...
        if (msg.vkCode < '0' || msg.vkCode > '9') {
            if (msg.vkCode == _T('-')) {
                if (GetTextLength() > 0) {
                    ...
                    //The minimum number is 0 or positive, sign input is forbidden
                    return true;
                }
            }
            else {
        ...
        if (!strClipText.empty()) {
            ...
            for (size_t index = 0; index < count; ++index) {
                ...
                if (!IsInLimitChars(strClipText[index])) {
                    //There are characters not in the list, pasting is forbidden
                    return true;
                }
            }
        }
        ...
        if (!strClipText.empty()) {
            ...
            for (size_t index = 0; index < count; ++index) {
                ...
                if ((strClipText[index] > L'9') || (strClipText[index] < L'0')) {
                    //There are non-digit characters, pasting is forbidden
                    return true;
                }
            }
        }
...
void RichEdit::Paint(IRender* pRender, const UiRect& rcPaint)
{
    ...
    UiRect rcTemp; //Dirty region within this control, the region that needs to be drawn this time
    if (!UiRect::Intersect(rcTemp, rcPaint, GetBoxShadowExpandedRect(GetRect()))) {//If the region including box-shadow is dirty, it needs to be drawn
        bNeedPaint = false;
    }
    ...
    if (bNeedPaint) {
        ...
        if(hdc != nullptr){
            ...
            //After drawing is complete, set a flag to avoid repeated drawing
            bNeedPaint = false;
        }
    }
    if (bNeedPaint) {
...
// An ultra-high-performance container dedicated to uint8_t, replacing std::vector<uint8_t>
// Reason: under C++17/VS2017, the std::vector<uint8_t> container has very poor performance
class RichEdit::FastBytes
{
public:
    // Default constructor: empty container, no memory allocation
    FastBytes() noexcept
    ...
    ~FastBytes() noexcept
    {
        // Using free is faster than delete[], with no destructor overhead
        if (m_data != nullptr) {
            std::free(m_data);
        }
    }

    // Copy is disabled (can be disabled for ultimate speed; implement it yourself if copying is needed)
    FastBytes(const FastBytes&) = delete;
    FastBytes& operator=(const FastBytes&) = delete;

    // reserve: pre-allocate memory without changing the size and without initialization (fastest)
    void reserve(size_t new_capacity) noexcept
    {
        ...
        // Allocate with raw malloc, much faster than vector's allocator
        uint8_t* new_data = static_cast<uint8_t*>(std::malloc(new_capacity));
        if (!new_data) {
            return; // Memory allocation failure is not handled (ultimate performance)
        }

        // Copy the existing data (only the valid data, no redundancy)
        if (m_data != nullptr) {
            if (m_size > 0) {
    ...
    // push_back: append an element at the end, no exceptions, no redundant checks
    inline void push_back(uint8_t value) noexcept
    {
        // Auto-expand when capacity is insufficient (1.5x expansion, uses less memory than 2x, similar speed)
    ...
    // size: return the current number of elements (inline, direct read)
    inline size_t size() const noexcept
    {
        return m_size;
    }

    // size: return the current capacity (inline, direct read)
    inline size_t capacity() const noexcept
    {
        return m_capacity;
    }

    // Fast direct access (faster than at, no checks, for internal use)
    ...
    {
        // Only set the size to 0, keep the memory, no release, no copy, no initialization
        // Speed = 1 instruction, much faster than vector
        m_size = 0;
    }

    //Clear the count to zero and release resources at the same time
...
private:
    uint8_t* m_data;    // Raw data pointer (no wrapper, fastest access)
    size_t   m_size;    // Current number of elements
    size_t   m_capacity;// Allocated memory capacity
};
...
void RichEdit::PaintRichEdit(IRender* pRender, const UiRect& rcPaint)
{
    ...
    if (clipType == RenderClipType::kEmpty) {
        //If the clip region is empty, no drawing is needed
        return;
    }
    ...
    if (!clipRects.empty()) {
        ...
        if (!bHasIntersect) {
            //The dirty region rectangle does not intersect the clip region rectangle, no drawing is needed
            return;
        }

        //Intersect with the clip region to avoid drawing overflow
    ...
    //Update region (relative to the top-left corner of the bitmap)
    rcUpdate.Offset(-rc.left, -rc.top);

    //Clear the Alpha channel (mark it as fully transparent)
    ...
    constexpr const int32_t nColorBits = sizeof(uint32_t); //Number of bytes occupied by each color pixel
    uint8_t* pRowStart = nullptr; //Starting position of the Alpha channel value of each row
    uint8_t* pRowEnd = nullptr;   //Ending position of the Alpha channel value of each row

    if (m_pAlphaValues == nullptr) {
        m_pAlphaValues = std::make_unique<FastBytes>();
    }
    FastBytes& alphaValues = *m_pAlphaValues; //Remember the original Alpha values in this region
    int32_t nEstSize = (nBottom - nTop) * (nRight - nLeft);
    if (nEstSize > 0) {
        if ((int32_t)alphaValues.capacity() > 2 * nEstSize) {
            //When the cache is too large, release the memory and reallocate appropriate memory to avoid excessive memory usage
            alphaValues.clear_release();
        }
        alphaValues.reserve(nEstSize);
    }
    bool bHasBkClolor = false; //Mark whether the background color has been set
    ...
    //The rectangular region of the bitmap
    RECTL rcBitmap = { 0, };
    rcBitmap.left = 0;
    rcBitmap.top = 0;
    ...
    //Set the clip information to avoid drawing non-updated regions
    ...
    //Restore Alpha (during drawing, the Alpha channel of the drawn region may become abnormal)
    size_t nAlpaIndex = 0;
    for (int32_t i = nTop; i < nBottom; ++i) {
        pRowStart = (uint8_t*)pBitmapBits + (i * nWidth + nLeft) * nColorBits + 3;
        pRowEnd = (uint8_t*)pBitmapBits + (i * nWidth + nRight) * nColorBits;
        while (pRowStart < pRowEnd) {
            if (*pRowStart == 0) {
                //The Alpha value is 0, meaning the drawing pierced through the background; the original Alpha value needs to be restored
                ASSERT(nAlpaIndex < alphaValues.size());
                if (nAlpaIndex < alphaValues.size()) {
                    *pRowStart = alphaValues[nAlpaIndex]; //Prefer the original Alpha value
                }
                else {
                    *pRowStart = 255; //Fault tolerance
                }
            }
            pRowStart += 4;
            ++nAlpaIndex;
    ...
    //Write the drawn data back to the render engine bitmap
    rcUpdate.Offset(rc.left, rc.top);
...
void RichEdit::DoSetRowSpacingMul(float fRowSpacingMul)
{
    ...
    pf2.dwMask = PFM_LINESPACING;            // This mask must be set to enable line spacing
    pf2.bLineSpacingRule = 5;                // Multiple line spacing mode
    pf2.dyLineSpacing = (LONG)(fRowSpacingMul * 20); // fRowSpacingMul times line spacing (fRowSpacingMul * 20)
    SetParaFormat(pf2);
}
        ...
        if (bEnable) {
            AttachContextMenu([this](const ui::EventArgs& args) {
                if (args.eventType == ui::kEventContextMenu) {
                    ...
                    if ((pt.x != -1) && (pt.y != -1)) {
                        //Context menu generated by a right mouse button click                        
                        ShowPopupMenu(pt);
                    }
                    else {
    ...
    if (IsNumberOnly()) {
        ...
        if (((GetMinNumber() != INT_MIN) || (GetMaxNumber() != INT_MAX))) {
            if (nNewValue > GetMaxNumber()) {
                ...
                //Less than the minimum number, correct it
...
bool RichEdit::FindRichText(const FindTextParam& findParam, TextCharRange& chrgText) const
{
    ...
    if (nIndex != -1) {
        //Content was found, select the found content
...
{
    m_bHideSelection = fHideSelection;//Record the state
    if (m_pRichHost != nullptr) {
...
void RichEdit::SetZoomPercent(uint32_t nZoomPercent)
{
    ...
    if ((nZoomPercent == 0) || (nZoomPercent == 100)) {
        ...
        //When an exact match is not possible, estimate a value
        int32_t nDen = 63;
