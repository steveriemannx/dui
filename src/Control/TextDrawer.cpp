                                 ...
                                 Control* pOwner)
{
    ...
    if (measureParam.uFormat & TEXT_VERTICAL) {
        //Vertical text, force disable RichText
        bRichText = false;
    }
    UiRect rcMessure;
    if (bRichText && (pOwner != nullptr)) {
        ...
        if (m_pRichText == nullptr) {
            ...
            m_pRichText->SetAttribute("trim_policy", "none"); //Do not perform trim operations on spaces
        }
        ...
        m_pRichText->SetWindow(pOwner->GetWindow()); //When drawing and parsing RichText, the associated window is needed (for DPI scaling, etc.)
                            ...
                            Control* pOwner)
{
    ...
    if (bRichText && (pOwner != nullptr)) {
        ...
        m_pRichText->SetEnableRedraw(false);
        m_pRichText->SetWindow(nullptr); //Do not set the associated window, to avoid Invalidate calls
        m_pRichText->SetRect(drawParam.textRect);//Do not call the SetPos function, to avoid events such as callbacks
