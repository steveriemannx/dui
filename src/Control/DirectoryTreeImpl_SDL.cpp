...
static DString DirectoryTreeImplGetImageString(DirectoryTree* pTree, bool bLargeFile, const DString& imageFileName)
{
    if (imageFileName.empty()) return DString();
...
bool DirectoryTreeImpl::GetVirtualDirectoryInfo(VirtualDirectoryType type, FilePath& filePath, DString& displayName, uint32_t& nIconID)
{
    ...
    case VirtualDirectoryType::kDesktop:
        filePath = userHomeDir; filePath += _T("Desktop");
    ...
    case VirtualDirectoryType::kDocuments:
        filePath = userHomeDir; filePath += _T("Documents");
        if (!filePath.IsExistsDirectory()) filePath = userHomeDir;
    ...
    case VirtualDirectoryType::kPictures:
        filePath = userHomeDir; filePath += _T("Pictures");
        if (!filePath.IsExistsDirectory()) filePath = userHomeDir;
    ...
    case VirtualDirectoryType::kMusic:
        filePath = userHomeDir; filePath += _T("Music");
        if (!filePath.IsExistsDirectory()) filePath = userHomeDir;
    ...
    case VirtualDirectoryType::kVideos:
        filePath = userHomeDir; filePath += _T("Videos");
        if (!filePath.IsExistsDirectory()) filePath = userHomeDir;
    ...
    case VirtualDirectoryType::kDownloads:
        filePath = userHomeDir; filePath += _T("Downloads");
        if (!filePath.IsExistsDirectory()) filePath = userHomeDir;
...
void DirectoryTreeImpl::GetRootPathInfoList(bool bLargeIcon, std::vector<DirectoryTree::PathInfo>& pathInfoList)
{
    ...
    for (char d = 'C'; d <= 'Z'; ++d) {
        ...
        if (std::filesystem::exists(drive, ec)) {
            ...
            pi.m_nIconID = GlobalManager::Instance().Icon().AddIcon(DirectoryTreeImplGetImageString(m_pTree, bLargeIcon, _T("drive-harddisk.svg")));
    ...
    for (auto &d : disks) {
        if (d.m_filePath == FilePath(_T("/"))) continue;
    ...
    try {
        ...
        for (const auto& entry : std::filesystem::directory_iterator(stdPath)) {
            ...
            if (isDir) {
                ...
                if (bLargeIcon) {
                    if (m_impl->m_nLargeFolderIconID == 0) m_impl->m_nLargeFolderIconID = GlobalManager::Instance().Icon().AddIcon(DirectoryTreeImplGetImageString(m_pTree, bLargeIcon, _T("folder.svg")));
                    pi.m_nIconID = m_impl->m_nLargeFolderIconID;
                } else {
                    if (m_impl->m_nSmallFolderIconID == 0) m_impl->m_nSmallFolderIconID = GlobalManager::Instance().Icon().AddIcon(DirectoryTreeImplGetImageString(m_pTree, bLargeIcon, _T("folder.svg")));
                ...
                if (entry.is_regular_file(ec)) {
                    ...
                    pi.m_displayName = pi.m_filePath.GetFileName();
                    if (bLargeIcon) {
                        if (m_impl->m_nLargeFileIconID == 0) m_impl->m_nLargeFileIconID = GlobalManager::Instance().Icon().AddIcon(DirectoryTreeImplGetImageString(m_pTree, bLargeIcon, _T("file.svg")));
                        pi.m_nIconID = m_impl->m_nLargeFileIconID;
                    } else {
                        if (m_impl->m_nSmallFileIconID == 0) m_impl->m_nSmallFileIconID = GlobalManager::Instance().Icon().AddIcon(DirectoryTreeImplGetImageString(m_pTree, bLargeIcon, _T("file.svg")));
...
void DirectoryTreeImpl::GetDiskInfoList(const std::weak_ptr<WeakFlag>&, bool bLargeIcon, std::vector<DirectoryTree::DiskInfo>& diskInfoList)
{
    ...
    if (!ec) {
        ...
        d.m_displayName = _T("/");
        d.m_filePath = FilePath(_T("/"));
        d.m_volumeName = _T("/");
        d.m_deviceType = DirectoryTree::DeviceType::HDD;
        d.m_mountOn = _T("/");
        d.m_fileSystem = _T("unknown");
        d.m_totalBytes = space.capacity;
        d.m_freeBytes = space.available;
        d.m_bIconShared = false;
        d.m_nIconID = GlobalManager::Instance().Icon().AddIcon(DirectoryTreeImplGetImageString(m_pTree, bLargeIcon, _T("drive-harddisk.svg")));
