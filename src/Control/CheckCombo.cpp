...
public:
    /** Create and show the drop-down window
    */
    ...
    */
    ...
    //Overrides of the base class virtual functions
...
void CCheckComboWnd::InitComboWnd(CheckCombo* pOwner)
{
    ...
    //Set the display position and size of the drop-down box to avoid a black screen when the popup interface appears
...
UiRect CCheckComboWnd::GetComboWndRect() const
{
    ...
    UiRect rc = rcOwner;
    rc.top = rc.bottom + Dpi().GetScaleInt(1);  // The left and bottom positions of the parent window serve as the popup window origin
    rc.bottom = rc.top + szDrop.cy;             // Calculate the popup window height
    if (szDrop.cx > 0) {
        rc.right = rc.left + szDrop.cx;         // Calculate the popup window width
    }

    //If all items in the child container are stretch types, the size does not need to be estimated (it would fail with an error); display according to the drop-down box setting instead
...
LRESULT CCheckComboWnd::OnKeyDownMsg(VirtualKeyCode vkCode, uint32_t modifierKey, const NativeMsg& nativeMsg, bool& bHandled)
{
    ...
    if (vkCode == kVK_ESCAPE) {
        //Hold down the ESC key to cancel
        CloseComboWnd();
    }
    return lResult;
}
...
void CheckCombo::Activate(const EventArgs* /*pMsg*/)
{
    ...
    m_pCheckComboWnd->AttachWindowCreateMsg(ToWeakCallback([this](const ui::EventArgs& /*args*/) {
        //Send the message to the application layer (drop-down window created)
        EventArgs msg;
        msg.SetSender(this);
    ...
    m_pCheckComboWnd->AttachWindowCloseMsg(ToWeakCallback([this](const ui::EventArgs& /*args*/) {
        //Send the message to the application layer (drop-down window destroyed)
        EventArgs msg;
        msg.SetSender(this);
