...
void CefBrowserHandler::SetViewRect(const UiRect& rc)
{
    ...
    // Call the WasResized interface. After the call, CefBrowserHandler will call the GetViewRect interface to obtain the new position of the browser object
                                                 ...
                                                 CefRefPtr<CefProcessMessage> message)
{
    ...
    // Handle messages sent from the render process
...
{
    // Let the new link open in the original browser object
    if (m_pHandlerDelegate && !m_bHostWindowClosed) {
        // Return true to keep opening new links in the control, false to forbid access
    ...
    // By default, popup windows are prohibited
    return true;
}
...
void CefBrowserHandler::OnAfterCreated(CefRefPtr<CefBrowser> browser)
{
    ...
    if (browser == nullptr) {
        ...
        std::lock_guard<std::mutex> threadGuard(m_dataMutex);
        // This window handle can be obtained on the TID_UI thread immediately, but it can no longer be obtained when the call is transferred to the main thread (there is a delay).
        auto hWndowHandle = browser->GetHost()->GetWindowHandle();//when the browser is dev tools, this returns nullptr, otherwise it is a non-null value
    ...
    GlobalManager::Instance().Thread().PostTask(ui::kThreadUI, ToWeakCallback([this, browser](){
        if (m_pHandlerDelegate != nullptr) {
            // In windowed mode, after the browser is created, let the upper layer update its own position; because in the asynchronous state, the Cef window may not have been created yet when the upper layer updates the position
...
void CefBrowserHandler::CloseAllBrowsers(bool bForceClose)
{
    std::vector<CefRefPtr<CefBrowserHost>> cefBrowserHostList;
    {
        ...
        // When there are multiple browsers, close them in reverse order
...
{
    ASSERT(CefCurrentlyOn(TID_UI));
    rect = { 0, 0, 1, 1 }; //Avoid returning 0, which would trigger a DCHECK error inside CEF
    if (m_bHostWindowClosed) {        
        return false;
    }
...
void CefBrowserHandler::GetViewRect(CefRefPtr<CefBrowser> browser, CefRect& rect)
{
    ...
    if (m_pHandlerDelegate) {
        ...
        if (CefManager::GetInstance()->IsEnableOffScreenRendering()) {
            // In off-screen rendering mode, the original width and height need to be passed because CEF will do further DPI scaling internally
...
bool CefBrowserHandler::GetScreenPoint(CefRefPtr<CefBrowser> browser, int viewX, int viewY, int& screenX, int& screenY)
{
    ...
    if (CefManager::GetInstance()->IsEnableOffScreenRendering()) {
        // In off-screen rendering mode, the given parameters are original coordinates without DPI scaling, so DPI scaling is required, otherwise the context menu position of the page is displayed incorrectly
        spWindow->Dpi().ScaleInt(screen_pt.x);
        spWindow->Dpi().ScaleInt(screen_pt.y);
    }
    // Convert the page coordinates to window client area coordinates, otherwise the position of the context menu popped up by the page is incorrect
    UiRect rect_cef_control;
    {
        std::lock_guard<std::mutex> threadGuard(m_dataMutex);
        rect_cef_control = m_rcCefControl;
...
{
    ASSERT(CefCurrentlyOn(TID_UI));
    // Only in off-screen rendering mode is the DPI scale factor handled
    if (!CefManager::GetInstance()->IsEnableOffScreenRendering()) {
        return false;
    }
    if (m_bHostWindowClosed) {
...
{
    // This function only has callback events in OSR mode
    if (m_pHandlerDelegate && !m_bHostWindowClosed) {
                                      ...
                                      int y)
{
    ...
    ASSERT(CefCurrentlyOn(TID_UI));
    // Function: drag-and-drop inside the web page starts (this interface is only called in off-screen rendering mode)
    if (!CefManager::GetInstance()->IsEnableOffScreenRendering()) {
        return false;
    }
#ifdef DUILIB_BUILD_FOR_WIN
    ...
    // Switch to the UI thread and perform the DoDragDrop operation
                                       ...
                                       const CefCursorInfo& custom_cursor_info)
{
    ...
    if (!CefManager::GetInstance()->IsEnableOffScreenRendering()) {
        // In child window mode, the cursor does not need to be set, otherwise the cursor will be abnormal
        return false;
    }
    else {
                                   ...
                                   bool& suppress_message)
{
    ...
    // Prevent the JS dialog from popping up in release mode
