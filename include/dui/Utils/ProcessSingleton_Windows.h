...
namespace ui
{
/** Cross-process singleton implementation (communication part, Windows implementation)
*/
    ...
    virtual void InitializePlatformComponents() override final
    {
        try {
            ...
            if (m_hCancelEvent == nullptr) {
                m_hCancelEvent = ::CreateEvent(nullptr, TRUE, FALSE, nullptr); // Manual-reset cancel event                
...
private:
    bool WaitForPipe(HANDLE hPipe, OVERLAPPED& overlapped, HANDLE hCancelEvent) const
    {
        ...
        bool bRet = false;
        switch (waitResult) {
            case WAIT_OBJECT_0: // Connection completed or canceled
                {
            ...
            case WAIT_OBJECT_0 + 1: // Cancel event triggered
                {
