...
#include <sys/stat.h>
#include <pwd.h>
#include <libproc.h>  // macOS-specific process-related header

namespace ui
{
/** Cross-process singleton implementation (macOS implementation)
*/
class DUI_API ProcessSingletonImpl: public ProcessSingleton
{
public:
...
public:
    virtual void InitializePlatformComponents() override final
    {
        try {
            ...
            // macOS requires a stricter temporary file path
            std::string strLockFile = strRuntimeDir + "/" + m_strAppName + ".lock";
            
            // macOS needs to unlink the existing file first
            ...
            // macOS recommends using fcntl instead of flock
            struct flock fl;
            fl.l_start = 0;
            fl.l_len = 0;
            fl.l_pid = getpid();
            ...
            // macOS needs to set file permissions explicitly
    ...
    virtual void PlatformListen() override final
    {
        try {
            ...
            // Handle the macOS socket path length limit
            std::string strSocketPath = GetUserRuntimePath() + "/" + m_strAppName + ".sock";
            if (strSocketPath.size() > sizeof(addr.sun_path) - 1) {
            ...
            // macOS needs to unlink the existing socket first
            ...
            // macOS needs to set permissions explicitly
            ...
            while (m_bRunning) {
                ...
                ssize_t nRead = ::recv(nClient, pBuffer, sizeof(pBuffer), 0);  // macOS does not need MSG_NOSIGNAL
                if (nRead > 0) {
                    try {
