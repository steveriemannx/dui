#ifndef UI_CORE_NATIVE_WINDOW_MACOS_H_
#define UI_CORE_NATIVE_WINDOW_MACOS_H_
 ...
 */
class DUI_API NativeWindow_MacOS
{
public:
    explicit NativeWindow_MacOS(INativeWindow* pOwner);
    NativeWindow_MacOS(const NativeWindow_MacOS& r) = delete;
    NativeWindow_MacOS& operator=(const NativeWindow_MacOS& r) = delete;
    ~NativeWindow_MacOS();
...
public:
    ...
    */
    bool CreateWnd(NativeWindow_MacOS* pParentWindow,
    ...
    */
    int32_t DoModal(NativeWindow_MacOS* pParentWindow,
                    const WindowCreateParam& createParam,
                    const WindowCreateAttributes& createAttributes,
    ...
    */
    bool CreateChildWnd(NativeWindow_MacOS* pParentWindow, int32_t nX, int32_t nY, int32_t nWidth, int32_t nHeight);
...
public:
    ...
    */
    void ShowModalFake(NativeWindow_MacOS* pParentWindow);

    /** The modal dialog closed; synchronize the state
    */
    void OnCloseModalFake(NativeWindow_MacOS* pParentWindow);
...
public:
    ...
    */
    bool SetWindowPos(const NativeWindow_MacOS* pInsertAfterWindow,
...
public:
    ...
    */
    ...
    /** Merge an AppKit-triggered dirty region (from drawRect) into the dui
    * update region so externally-triggered redraws (expose, resize) repaint.
    * The rect is in client (pixel) coordinates.
    */
    void OnNativeMergeDirtyRect(const UiRect& rcDirty);
...
public:
    /** @name AppKit event bridge (called from the NSView/NSWindow delegate helpers)
    * @{ */
    /** Handles an AppKit event for this window (called from the NSView)
    * @param [in] pEvent The NSEvent* pointer
    * @return true if the event was handled
    */
    bool OnNativeEvent(void* pEvent);

    /** The NSWindow finished resizing
    */
    void OnNativeWindowDidResize();
    ...
    */
    void OnNativeWindowBecomeKey();

    /** The NSWindow resigned key window status
    */
    void OnNativeWindowResignKey();

    /** The NSWindow is about to close (traffic light close button or [NSWindow close])
    */
    void OnNativeWindowWillClose();

    /** Processes the deferred finalize message (posted from OnNativeWindowWillClose)
    */
    void OnNativeFinalizeFromMessage();

    /** The NSWindow entered fullscreen (state comes from the delegate, not the toggle call)
    */
    void OnNativeWindowDidEnterFullscreen();

    /** The NSWindow exited fullscreen (also restores the system shadow, macOS quirk)
    */
...
public:
    /** @name Drag-related interfaces (macOS NSDraggingDestination bridge)
    * @{ */
    void OnDropBegin();
...
private:
    ...
    */
    bool CreateWindowAndRender(NativeWindow_MacOS* pParentWindow, const WindowCreateAttributes& createAttributes);
    ...
    */
    ...
    /** Gets the monitor rectangle and work area rectangle of the monitor containing the window
    * @param [out] rcMonitor The monitor rectangle
    ...
    */
    ...
    /** Creates the Render (Skia) associated with the window
    */
    IRender* CreateRender();

    /** Calculates the centered window position
    */
    bool CalculateCenterWindowPos(int32_t& xPos, int32_t& yPos) const;
...
private:
    ...
    */
    ...
    /** NSWindow* (Cocoa window)
    */
    void* m_nsWindow;

    /** NSView* (content view hosting the render)
    */
    void* m_nsView;

    /** NSTimer* that coalesces repaints to ~60 Hz (keeps CPU rendering
    * responsive: without it every Invalidate repaints synchronously and a
    * mouse move storm saturates the CPU)
    */
    void* m_displayTimer;

    /** The Skia render interface
    */
    IRender* m_pRender;
    ...
    */
    ...
    /** Parent native window (for re-attaching macOS child windows after they
    *  are hidden with orderOut, which AppKit detaches from the parent).
    */
    NativeWindow_MacOS* m_pParentNativeWindow;
    ...
    */
    ...
    /** The cached window rect (used when dispatching move messages)
    */
    UiRect m_rcWindowRect;

    /** The retained NSWindowDelegate* (DuIWindowDelegate); NSWindow.delegate is weak,
    * so the dui side must keep a strong reference for the lifecycle to survive.
    */
    void* m_delegate = nullptr;

    /** Whether the deferred finalize has run (guards double destruction)
    */
    bool m_bFinalized = false;

    /** Whether PaintWindow is currently executing (Invalidate re-entrancy guard)
    */
    bool m_bPainting = false;

    /** An Invalidate arrived during PaintWindow; needs one more display pass
    */
    bool m_bPendingPaint = false;

    /** The window rect before entering fullscreen (restored by ExitFullscreen)
    */
    UiRect m_rcBeforeFullscreen;

    /** The window level before entering fullscreen (restored by ExitFullscreen)
    */
    int32_t m_levelBeforeFullscreen = 0;

    /** The OS shadow type before entering fullscreen (restored by ExitFullscreen;
...
/** Defines an alias
*/
typedef NativeWindow_MacOS NativeWindow;

} // namespace ui

#endif //DUI_BUILD_FOR_MACOS

#endif // UI_CORE_NATIVE_WINDOW_MACOS_H_
