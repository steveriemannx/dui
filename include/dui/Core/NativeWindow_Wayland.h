#ifndef UI_CORE_NATIVE_WINDOW_Wayland_H_
#define UI_CORE_NATIVE_WINDOW_Wayland_H_
...
//Wayland types, forward declared
struct Wayland_Window;
struct Wayland_Renderer;
...
struct wl_callback;
#endif
typedef uint32_t Wayland_WindowID;
typedef uint64_t Wayland_WindowFlags;
typedef uint32_t Wayland_Keycode;
typedef uint16_t Wayland_Keymod;
typedef uint32_t Wayland_PropertiesID;
union Wayland_Event;
...
*/
class DUI_API NativeWindow_Wayland
{
public:
    explicit NativeWindow_Wayland(INativeWindow* pOwner);
    NativeWindow_Wayland(const NativeWindow_Wayland& r) = delete;
    NativeWindow_Wayland& operator=(const NativeWindow_Wayland& r) = delete;
    ~NativeWindow_Wayland();

public:
    /** Gets the window ID associated with the Wayland event
    * @return Returns 0 if there is no associated window ID
    */
    static Wayland_WindowID GetWindowIdFromEvent(const Wayland_Event& nativeEvent);

    /** Gets the window pointer by the window ID
    */
    static NativeWindow_Wayland* GetWindowFromID(Wayland_WindowID id);
    ...
    * @param [in] nativeEvent The message data
    * @return Returns true if the message was handled internally, otherwise false
    */
    bool OnWaylandWindowEvent(const Wayland_Event& nativeEvent);
...
public:
    ...
    */
    bool CreateWnd(NativeWindow_Wayland* pParentWindow,
    ...
    */
    int32_t DoModal(NativeWindow_Wayland* pParentWindow,
                    const WindowCreateParam& createParam,
                    const WindowCreateAttributes& createAttributes,
    ...
    */
    bool CreateChildWnd(NativeWindow_Wayland* pParentWindow, int32_t nX, int32_t nY, int32_t nWidth, int32_t nHeight);
    ...
    */
    bool SetParentWindow(NativeWindow_Wayland* pParentWindow);
...
public:
    ...
    */
    void ShowModalFake(NativeWindow_Wayland* pParentWindow);

    /** The modal dialog closed; synchronize the state
    */
    void OnCloseModalFake(NativeWindow_Wayland* pParentWindow);
...
public:
    ...
    */
    bool SetWindowPos(const NativeWindow_Wayland* pInsertAfterWindow,
...
public:
    ...
    */
    ...
    /** The Wayland Hit Test callback function
    */
    int32_t Wayland_HitTest(Wayland_Window* win, const Wayland_Point* area, void* data);
    ...
    */
    ...
    /** Restricts pointer input to the drawn window, excluding self-drawn shadow margins. */
    void UpdateWaylandInputRegion();
...
private:
    ...
    */
    bool CreateWindowAndRender(NativeWindow_Wayland* pParentWindow, const WindowCreateAttributes& createAttributes);
    ...
    */
    bool GetMonitorRect(Wayland_Window* nativeWindow, UiRect& rcMonitor, UiRect& rcWork) const;

    /** Gets the window rect of the current window
    * @param [out] rcWindow Returns the screen coordinates of the window top-left and bottom-right corners
    */
    void GetWindowRect(Wayland_Window* nativeWindow, UiRect& rcWindow) const;
    ...
    */
    void SetCreateWindowProperties(Wayland_PropertiesID props,
                                   NativeWindow_Wayland* pParentWindow,
                                   const WindowCreateAttributes& createAttributes,
                                   bool bUseOpenGL);

    /** Creates the Render associated with the Wayland window
    */
    Wayland_Renderer* CreateWaylandRenderer(const std::string& nativeRenderName) const;
    ...
    */
    bool CalculateCenterWindowPos(Wayland_Window* pCenterWindow, int32_t& xPos, int32_t& yPos) const;

    /** Checks the window edge-snap operation and calls back to the application layer
    */
    void CheckWindowSnap(Wayland_Window* window);

private:
    /** Sets the mapping between the window ID and the window pointer
    */
    static void SetWindowFromID(Wayland_WindowID id, NativeWindow_Wayland* pNativeWindow);

    /** Clears the mapping between the window ID and the window pointer
    */
    static void ClearWindowFromID(Wayland_WindowID id, NativeWindow_Wayland* pNativeWindow);

    /** Converts an Wayland key to the internal ModifierKey
    */
    static uint32_t GetModifiers(Wayland_Keymod keymod);
...
private:
    ...
    * @{ */
    ...
    /** Wayland_EVENT_DROP_BEGIN
    */
    void OnDropBegin();

    /** Wayland_EVENT_DROP_POSITION
    * @param [in] pt A point in client coordinates
    ...
    */
    ...
    /** Wayland_EVENT_DROP_COMPLETE or another message that causes the leave
    */
    void OnDropLeave();
...
private:
    /** The mapping between window pointers and Wayland window IDs, used to relay messages
    */
    static std::unordered_map<Wayland_WindowID, NativeWindow_Wayland*> s_windowIDMap;
    ...
    */
    ...
    /** Wayland window
    */
    Wayland_Window* m_nativeWindow;

    /** The Wayland window rendering interface
    */
    Wayland_Renderer* m_nativeRenderer;
    ...
    */
    Wayland_WindowFlags m_lastWindowFlags;
...
/** Defines an alias
*/
typedef NativeWindow_Wayland NativeWindow;

} // namespace ui

#endif //DUI_BUILD_FOR_WAYLAND

#endif // UI_CORE_NATIVE_WINDOW_Wayland_H_
