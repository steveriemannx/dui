...
| :--- | :--- | :--- |
| Interface class | [include/duilib/duilib_cef.h](../include/duilib/duilib_cef.h) | The interface class of the CEF control; applications need to include this header file: `#include "duilib/duilib_cef.h"` |
...
The CMake build downloads the libcef binaries automatically at configure time when they are missing (into the `${NIM_DUILIB_ROOT}\third_party\libcef\cef_binary` directory; see `cmake/duilib_deps.cmake`).    
...
5. Place the libcef binaries and resource files (libcef.dll, etc.) in the runtime directory of your project (for example the x64 version in `${NIM_DUILIB_ROOT}\bin\libcef_win\x64` and the Win32 version in `${NIM_DUILIB_ROOT}\bin\libcef_win\Win32`).
...
4. In the VC project, set libcef.dll to be delay-loaded by adding `libcef.dll` (entry: VS project properties -> Linker -> Input -> Delay Loaded Dlls)    
5. Place the libcef binaries and resource files (libcef.dll, etc.) in the runtime directory of your project (for example the x64 version in `${NIM_DUILIB_ROOT}\bin\libcef_win_109\x64` and the Win32 version in `${NIM_DUILIB_ROOT}\bin\libcef_win_109\Win32`).
...
### 1. Getting the libcef binaries and resource files (automatic with CMake)
The CMake build downloads the libcef binaries automatically at configure time when they are missing (into the `${NIM_DUILIB_ROOT}/third_party/libcef/cef_binary` directory); the `${NIM_DUILIB_ROOT}/bin/libcef_linux/` directory is used as the runtime library directory.    
...
The basic steps are as follows (all directories are written as subdirectories relative to the nim_duilib root `${NIM_DUILIB_ROOT}`; the actual settings can be flexibly adjusted according to your own project structure):    
### 1. Getting the libcef binaries and resource files (automatic with CMake)
The CMake build downloads the complete binary distribution automatically at configure time when it is missing, and extracts it to the `${NIM_DUILIB_ROOT}/third_party/libcef/cef_binary` directory (it contains the framework binaries in the `Release/` directory).    
Note: `third_party/libcef/libcef_macos` is released together with the nim_duilib source code (wrapper sources and headers only); `cef_binary` is the complete binary distribution.    
`cmake/duilib_cef_macos.cmake` uses the `cef_binary` directory as the default `CEF_ROOT`; it can be overridden with `-DCEF_ROOT=...` when running CMake.
