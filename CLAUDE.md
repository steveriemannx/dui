# nim_duilib - Cross-Platform C++ UI Library

## Project Overview
nim_duilib is a cross-platform C++ UI framework based on the Skia rendering engine, using a development model of XML-described UI layout + C++ control logic.
...
```
nim_duilib/
├── duilib/              # core library source code
...
**Single entry point (Qt-style):** every example/app has one `main.cpp` using the entry macro from `duilib/Utils/AppEntry.h`; the platform entry function (wWinMain/WinMain/main) is generated automatically:

```cpp
#include "TestApplication.h"
#include "duilib/Utils/AppEntry.h"

DUILIB_APP_ENTRY(TestApplication)        // AppClass must provide void Run();
// DUILIB_APP_ENTRY_ARGS(AppClass)       // AppClass must provide static Instance() + int Run(int argc, char** argv)
```
...
- Detailed LLM reference: `.claude/docs/nim-duilib-llm-reference.md`
...
- Cross-platform: `scripts/build_duilib_all_in_one.sh` or `scripts/build_duilib_all_in_one.bat`
- Dependencies: Skia must be compiled first (see `scripts/build.md`)
- Example mode selection (CMake): `-DDUILIB_EXAMPLES_MODE=ALL|XML|GEN|CODE` — builds only the examples of one development mode (XML / XML-to-code generation / pure code); default `ALL`
