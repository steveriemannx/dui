...
if [ "$(uname -s)" == "Darwin" ]; then
    echo "macOS"
    chmod +x ./nim_duilib/scripts/macos_build.sh
    ./nim_duilib/scripts/macos_build.sh
elif [ "$(uname -s)" == "FreeBSD" ]; then
    echo "FreeBSD"
    chmod +x ./nim_duilib/scripts/freebsd_build.sh
    ./nim_duilib/scripts/freebsd_build.sh
elif is_windows; then
    echo "Windows"
    ./nim_duilib/scripts/msys2_build.sh $1
else
    echo "Linux"
    if [ "$ENABLE_WAYLAND" == "1" ]; then
        echo "  using Wayland backend"
        chmod +x ./nim_duilib/scripts/linux_build_wayland.sh
        ./nim_duilib/scripts/linux_build_wayland.sh
    else
        chmod +x ./nim_duilib/scripts/linux_build.sh
        ./nim_duilib/scripts/linux_build.sh
    fi
fi
