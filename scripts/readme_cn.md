...
本文档详细介绍 `nim_duilib\scripts` 目录中 Visual Studio 解决方案文件（*.sln）和编译脚本文件（*.bat/*.sh）的用途及使用方法，供维护人员参考。
...
│       └── ...
│
├── scripts/build_temp/                   # 编译临时目录（可安全删除）
│   ├── msvc/
│   └── ...
│
└── scripts/.vs/                          # VS 缓存目录（可定期清理）
```
...
```batch
# 清理 VS 缓存
rmdir /s /q nim_duilib\scripts\.vs

# 清理编译临时文件
rmdir /s /q nim_duilib\scripts\build_temp
